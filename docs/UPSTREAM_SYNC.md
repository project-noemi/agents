...
#    - Keep my organization's version where they diverge
...
1. Open the cloned `MyOrganization/agents` repository as a workspace in Antigravity.
2. Create a **Workflow** (saved prompt) for reuse. In Antigravity, workflows are saved prompts you trigger with `/`. Create one called `sync-upstream`:
   ...
   **Prompt:**
   ...
   6. If conflicts occur, stop and list the conflicting files. Our policy: keep my organization's version.
...
3. Optionally, add a **Rule** to Antigravity that applies to this workspace:
   ...
   **Rule:**
   ```
   This repository (MyOrganization/agents) tracks project-noemi/agents as its
   upstream source. When resolving merge conflicts between upstream and local changes,
   always preserve my organization's version. The upstream remote URL is:
...
(MyOrganization/agents) tracks project-noemi/agents as upstream.
...
5. If there are conflicts, stop and show me which files conflict. Our policy is to keep my organization's version.
...
function sendSyncReminder() {
  var admin = 'admin@MyOrganization.com'; // change to admin's email
  var subject = '[Agents Repo] Weekly upstream sync due';
  var body = [
    'It\'s time for the weekly upstream sync of MyOrganization/agents.',
    '',
    'Steps:',
...
```bash
git clone git@github.com:MyOrganization/agents.git
cd agents
