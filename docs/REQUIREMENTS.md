...
- **Fleet Report Schema Mismatch (Skill vs. Implementation)**: `skills/reporting/structured-report.md` mandates a JSON report shape with `cycle_timestamp`, `summary.total_evaluated`, nested `summary.actions.*`, and a `details[]` array, and claims to follow "the Fleet Dashboard ingestion schema". However, the actual reference pipeline uses a different shape: `examples/gatekeeper-deployment/entrypoint.sh` emits `cycle_started_at`/`cycle_completed_at`, `repos_scanned`, `prs_evaluated`, and a flat `summary.*`, which is what `dashboard-ingest.js` reads. Because `buildInfluxLine()` defaults missing fields to `0`/`unknown`, a skill-compliant report is accepted (HTTP 202) but recorded as all-zero metrics — a silent data-loss path for any agent that composes the skill as written.
- **Slack Block Kit Formatting Drift**: The Slack MCP protocol and the `skills/reporting/alert-notify.md` skill both mandate Block Kit formatting (with severity levels and an agent-ID/timestamp footer) for Slack notifications. The Gatekeeper reference implementation (`examples/gatekeeper-deployment/entrypoint.sh`, `post_slack_summary()`) posts a plain `{"text": "..."}` webhook payload with no Block Kit structure, severity level, or footer, despite the Gatekeeper persona referencing `reporting/alert-notify` for this exact step.
- **Non-Timing-Safe Bearer Token Comparison**: `examples/gatekeeper-deployment/dashboard-ingest.js` deliberately uses `crypto.timingSafeEqual` for HMAC signature verification, but compares the `Authorization: Bearer` API token with a plain `!==` string comparison (line ~98). The inconsistent hardening posture within the same security-focused reference service weakens its value as a copy-paste baseline.
