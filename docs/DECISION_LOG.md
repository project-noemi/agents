...
## [2026-03-01] - Formal Resolution of Architectural and Documentation Drifts
- **Decision**: Archived all resolved clarifications and integrated them into the core requirements.
- **Context**: Finalizing the "Doc" phase by cleaning up the clarification backlog and ensuring the source of truth reflects all human feedback.
- **Impact**:
    - Standardized agent persona template to "Role, Mission, Core Mandates, Workflow, Boundaries" in `REQUIREMENTS.md`.
    - Confirmed Project NoéMI branding across primary documentation (`REQUIREMENTS.md`, `GEMINI.template.md`, and generated `GEMINI.md`).
    - Resolved the stateless vs. persistent memory layer contradiction in `REQUIREMENTS.md`.
    - Defined Docker and Gemini CLI as required for local development/pre-flight in `REQUIREMENTS.md`.
    - Synchronized `docs/agents/` hierarchy with `agents/` by creating missing `marketing/`, `engineering/`, and `operations/` directories.
- **Resolved Questions**:
