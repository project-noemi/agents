...
- **Merge Gate Wildcard Regression ("CI Deadlock" Fix)**: Commit `a0a7fad` (2026-07-07, "Fix CI deadlock") re-added a `^doc-requirements-uplift-` exception to `.github/workflows/require-develop-source.yml`, directly contradicting Decision [2026-07-03-0001] — which removed exactly that wildcard family and states any hotfix path must be "a single explicit, documented exception — never a broad `doc-*` wildcard" — while the workflow's own inline comment still describes the strict develop-only behavior its code no longer enforces. PR #273 merged directly into `main` through this exception, diverging `main` from `develop` again (reconciled by sync-down on 2026-07-08) and forking the documentation set: the direct-to-main doc-uplift run left `main` carrying eleven clarification questions the develop line does not — nine already closed on develop via the [2026-06-19] and [2026-07-05] decision series, plus two newly added duplicates of approved Decisions [2026-06-19-0005] and [2026-06-19-0012] (see Clarification [2026-07-08] Merge Gate Wildcard Regression).
...
- **Operating Profile Starter Set Incomplete (2 of 3)**: Decision [2026-06-19-0011] approved three baseline Operating Profiles. `standard-operating-profile.md` shipped on 2026-07-05 (commit `9c19548`) and is injected into generated context, but `high-trust-regulated.md` and `fast-iteration-startup.md` remain unauthored.
...
- **Infrastructure Asset Implementation Gap** (Remediated: 2026-07-05, verified 2026-07-08): `templates/tiers/` now ships substantive `basic.md`, `standard.md`, and `premium.md` service tier templates, and `operating-profiles/standard-operating-profile.md` exists and is injected into the `OPERATING_PROFILE_INJECTIONS` block of `GEMINI.md`/`CLAUDE.md` (commit `9c19548`, salvaged from stale PR #254). The tier-template and hollow-injection portions of this gap are closed; the two remaining approved profiles are tracked above as "Operating Profile Starter Set Incomplete".
