...
- **Logging Protocol Implementation Gap**: There is a technical mismatch between the `logging-mcp` definition (Loki/n8n backends, standardized JSON) and the `Fleet Dashboard` reference implementation (`dashboard-ingest.js` using InfluxDB line protocol), hindering unified observability.
- **Config-to-Asset Mapping Drift**: `scripts/audit-repo.js` does not verify that all active skills and MCPs listed in `mcp.config.json` have corresponding files in `skills/` and `mcp-protocols/`, allowing for broken references in generated context.
