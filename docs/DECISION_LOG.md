...
## [2026-07-08-0001] Tier Template Clarifications Closed as Overtaken by Events

- **Decision:** Q [2026-06-10] Empty Tier Templates Implementation Gap and Q [2026-06-15] Service Tier Template Specifications are closed as overtaken by events and removed from `docs/CLARIFICATIONS.md`. Commit `9c19548` (2026-07-05, "salvage gap-filling assets from stale PR #254") shipped substantive `templates/tiers/basic.md`, `standard.md`, and `premium.md` (positioning, provisioned MCPs, limits per tier) plus `operating-profiles/standard-operating-profile.md`, which now injects into the `OPERATING_PROFILE_INJECTIONS` block of `GEMINI.md`/`CLAUDE.md`. Both questions asked for authorization or inputs to create files that now exist with real content, so no product-owner input remains outstanding.
- **Context:** Verified on 2026-07-08 against the working tree: all three tier templates present with substantive content, standard profile present and injected (2 occurrences in each generated context file). The Client Onboarding workflow's Explorer path is no longer blocked on missing templates.
- **Impact:** `docs/CLARIFICATIONS.md` shrinks by two questions. `docs/REQUIREMENTS.md` moves "Infrastructure Asset Implementation Gap" to the Remediated archive and replaces it with a narrower open entry, "Operating Profile Starter Set Incomplete (2 of 3)": `high-trust-regulated.md` and `fast-iteration-startup.md` from Decision [2026-06-19-0011] remain unauthored.

## [2026-07-08-0002] Sync-Down Reconciliation After the PR #273 Direct-to-Main Merge; Merge Gate Regression Recorded

- **Decision:** `origin/main` is merged back into the develop line to restore the develop-superset invariant, with `docs/CLARIFICATIONS.md` and `docs/REQUIREMENTS.md` conflicts resolved in favor of the develop line. The two questions PR #273 added on `main` (audit-script "TBD" placeholder rejection; Agent Index regex refinement for technical names) are not re-imported: both restate decisions already approved on the develop line — Decision [2026-06-19-0005]/[2026-06-19-0012] (substantive audit checks and full-first-paragraph extraction, approved scope, implementation pending). The merge-gate wildcard regression itself (commit `a0a7fad` re-adding `^doc-requirements-uplift-` to `.github/workflows/require-develop-source.yml` against the explicit prohibition in Decision [2026-07-03-0001]) is left operationally as-is pending product-owner direction and is recorded as a Current Known Limitation plus Clarification [2026-07-08] Merge Gate Wildcard Regression; a companion Clarification [2026-07-08] raises the underlying dual doc-automation split (Jules doc-uplift on `main` vs. the Claude Doc routine on `develop`).
- **Context:** PR #273 (Jules doc-uplift bot) merged directly into `main` on 2026-07-07 through the re-added wildcard, diverging `main` from `develop` by four commits and leaving `main` carrying eleven clarification questions the develop line does not — nine already closed on develop via the [2026-06-19] and [2026-07-05] decision series, plus the two new duplicates named above. This is the same divergence-then-sync-down cycle documented in Decision [2026-07-03-0001] (the #248/#250/#252/#255 sequence). Reverting the gate unilaterally was rejected for this pass: CI gate behavior is a governance decision, and removing the exception without repointing the doc-uplift bot would re-create the "CI deadlock" the commit author was working around.
- **Impact:** The develop line again contains everything on `main`. `docs/REQUIREMENTS.md` gains the "Merge Gate Wildcard Regression" Known Limitation and updates the develop-only invariant description to reflect the regression. Two new clarifications dated [2026-07-08] capture the decisions needed. Note for the pending PR #275 reviewer: `examples/red-team-gauntlet/` now ships BOTH `test-vectors.yaml` and `test-vectors.json` (the JSON arrived in `9c19548`), which is new evidence for the Red Team format-conflict question #275 resolves.
