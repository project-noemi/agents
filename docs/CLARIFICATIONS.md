...
### ❓ Question [2026-06-10] - Canonical Exponential Backoff Parameters
**Context:** Requirement §8 and `AGENTS.md` ("Exponential Backoff") mandate exponential-backoff retry logic and name `scripts/resilience_helpers.js` as the canonical Node.js reference. The repository also ships `scripts/retry-with-backoff.sh` (acknowledged in Decision [2026-04-02] for non-Node runtimes), but no document defines the canonical retry parameters.
**Ambiguity / Drift:** The two in-repo references diverge: `resilience_helpers.js` defaults to `maxRetries=5`, `baseDelayMs=1000`, `maxDelayMs=30000`, and `jitterFactor=0.2` (jitter present), while `retry-with-backoff.sh` defaults to `RETRY_MAX_ATTEMPTS=5`, `RETRY_INITIAL_DELAY_SECONDS=1`, `RETRY_MAX_DELAY_SECONDS=16`, and applies **no jitter**. Agent-facing git-operation guidance elsewhere prescribes a third profile (4 attempts; 2s/4s/8s/16s). Without a canonical profile, forks and agents implement inconsistent resilience behavior under load.
**Question for Product Owner:** Should Requirement §8 / `AGENTS.md` document a single canonical backoff profile (max attempts, base delay, max-delay cap, and whether jitter is mandatory), and should `retry-with-backoff.sh` be aligned to it (e.g., add jitter, match the delay cap) or explicitly scoped as a deliberately distinct shell/CI profile?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Define a canonical exponential-backoff parameter table in Requirement §8 and `AGENTS.md`, then reconcile `scripts/resilience_helpers.js` defaults and `scripts/retry-with-backoff.sh` to it (or document the shell variant's divergence as intentional).*

### ❓ Question [2026-06-10] - Branch-Protection Flow Exceptions in CI
**Context:** Requirement §7 mandates the canonical `develop → main` flow, and `.github/workflows/require-develop-source.yml` enforces it by blocking PRs to `main` whose head branch is not `develop`. The same workflow, however, hardcodes an allowlist of branch-name prefixes (`doc-workflow-audit-drift*`, `doc-requirements-refinement*`, `doc-requirements-drift-audit*`, `doc-reality-check*`, `doc-requirements-update*`) that may target `main` directly, bypassing `develop`.
**Ambiguity / Drift:** These exceptions are not recorded in `REQUIREMENTS.md` or `DECISION_LOG.md`, so the "mandatory `develop → main`" governance statement and the enforced reality disagree. The allowlist is also brittle: it is coupled to specific Doc branch-name conventions (and does not, for example, cover the `claude/*` automation branch pattern), so the set of sanctioned exceptions drifts silently as branch naming evolves.
**Question for Product Owner:** Should the `develop → main` exception policy be formally documented (in §7 and `DECISION_LOG.md`) with a stable, intentional rule — e.g., a single reserved prefix or a label-based bypass — rather than an ad-hoc list of Doc branch prefixes embedded in CI?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Document the sanctioned `develop → main` bypass policy in Requirement §7 and `DECISION_LOG.md`, and refactor `.github/workflows/require-develop-source.yml` to read the exception rule from a single documented convention (reserved prefix or PR label) instead of a hardcoded branch-prefix list.*
