...
- **Structural vs. Substantive Compliance**: Across the 26-agent fleet, most personas still use identical boilerplate for their safety and observability sections — 23/26 share the generic `Data Inventory` line, 25/26 share the boilerplate `### Refusal Criteria`, and 25/26 retain the literal `Audit Log` JSON placeholder (`"task": "..."`). This satisfies structural audits but fails framework requirements for role-specific precision.
...
- **Audit Script Enforcement Depth** (Partial — Decision [2026-05-20] not fully shipped): `scripts/audit-repo.js` now audits `skills/` headings and parses the `Audit Log` JSON block, but two of the three mandates from Decision [2026-05-20] (Audit Script Coverage Expansion) remain unimplemented: (1) it does not validate the `Audit Log` JSON against the canonical schema (`task`/`inputs`/`actions`/`risks`/`result` keys) — it only runs `JSON.parse`; and (2) it does not emit its own JSON Audit Log to `stderr`. It also still does not verify that skills or MCPs referenced in `mcp.config.json` and agent workflows exist as physical files.
...
- **Fleet Dashboard Reference Implementation Location Drift**: The Fleet Dashboard persona's reference implementation (`dashboard-ingest.js`) is colocated in `examples/gatekeeper-deployment/`, and no `examples/fleet-dashboard/` directory exists. Decision [2026-05-20] (Fleet Dashboard Ingestion Path) references a non-existent `examples/fleet-dashboard/dashboard-ingest.js`, so the durable audit trail points at a path that cannot be resolved. The `/api/v1/reports` path itself is correctly implemented in the actual file and asserted by `tests/examples-smoke.test.js`.
- **Coding Domain Documentation Mirror Gap**: The `coding` domain contains five specifications (`architect/core.md`, `bolt/core.md`, `bolt/go.md`, `bolt/nextjs-16.md`, `sentinel/core.md`), but `docs/agents/coding/` holds only a `README.md` plus empty `.gitkeep` placeholders for `bolt/` and `sentinel/`, with no directory for `architect/`. Even under the directory/guide-level mirror interpretation (Decision [2026-04-02]), the coding domain carries no guide content and `architect` has no documentation presence.
- **Undocumented Agent Variant Convention**: The fleet has grown a "core + framework specialization" pattern (`bolt/core.md`, `bolt/go.md`, `bolt/nextjs-16.md`) surfaced as distinct entries in the Agent Index, but no governance rule in `AGENTS.md` or `REQUIREMENTS.md` defines how variant specs relate to their core spec (naming convention, required-section duplication, or shared vs. overridden content).
