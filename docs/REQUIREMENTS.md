...
- **Backoff Parameter Drift**: The repository ships two retry helpers with divergent contracts. `scripts/resilience_helpers.js` caps delay at `30000` ms with a `0.2` jitter factor over 5 retries, while `scripts/retry-with-backoff.sh` caps at `16` s with no jitter over 5 attempts. Requirement §8 names only the Node helper and defines no canonical backoff parameters, leaving the authoritative delay ceiling and jitter policy ambiguous (open clarification [2026-06-07]).
- **Undocumented Executive Assistant Triage Contract**: `tools/executive-assistant/triage-router.js` implements a three-way classification (`IGNORE` → `VIP` → `STANDARD`, precedence locked by `tools/executive-assistant/tests/triage-router.test.js`), but this business logic is not represented in the requirements, which mention `executive-assistant` only as an observability gap (open clarification [2026-06-07]).
- **Test Framework Drift (Vitest)**: `tools/executive-assistant` uses Vitest (`"test": "vitest run"`) rather than the built-in Node test runner mandated as the primary validation framework. Its tests under `tools/executive-assistant/tests/` are not matched by the root `tests/*.test.js` glob, so they are excluded from `npm run validate` and the documented CI gate (open clarification [2026-06-07]).
