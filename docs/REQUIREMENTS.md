...
- **Merge-Gate Wildcard Re-Accretion and develop/main Divergence**: Requirement §7 mandates that `develop` is the only valid PR source into `main` and that any hotfix exception must be "a single explicit entry... never a broad wildcard," but `main`'s copy of `.github/workflows/require-develop-source.yml` has re-accreted two wildcard exceptions (`doc-requirements-uplift-*` and `doc-requirements-refinement-*`) via direct-to-main automation commits `a0a7fad` (2026-07-07) and `e54275a` (2026-07-09) — after Decision [2026-07-03-0001] removed the previous six. As of 2026-07-10 the branches have diverged (8 commits only on `develop`, 7 only on `main`), and the two branches carry conflicting `CLARIFICATIONS.md`/`DECISION_LOG.md`/`REQUIREMENTS.md` states: `main`-side bot runs re-raised questions already resolved on `develop` by the [2026-07-07-000x] decision series. The weekly `develop → main` release PR mandated by §7 has not run since the invariant was codified (see Clarification [2026-07-10] Merge-Gate Wildcard Re-Accretion).
...
- **Ungoverned Value-Lens JSON (compassion-lens)**: `value-lenses/compassion-lens.json` (commit `bb381d2`, "precarity-consciousness and anti-replacement alignment") is a JSON-only value lens whose stated intent is to inject its values fleet-wide, but the generator injects only `.md` lenses, so it never reaches generated context. It also references a "Project NoéMI Anti-Replacement rule" that is defined nowhere in `REQUIREMENTS.md`, `AGENTS.md`, or the framework docs, while three personas (`agents/guardian/roi-auditor.md`, `agents/education/student-success-coach.md`, `agents/operations/qa-risk-manager.md`) already carry constraints derived from it (Workforce Uplift Constraint, Precarity Consciousness, Contextual Forgiveness Rule). Unlike `american-dream` (canonical `.md` lens with a `.json` machine-readable companion), the compassion lens exists only in the ungoverned JSON form (see Clarification [2026-07-10] Compassion Lens Governance).
