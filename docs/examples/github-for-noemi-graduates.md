...
> A practical decision guide for Project NoéMI graduates who are new to GitHub and more familiar with SharePoint.
...
It follows the same patterns used in this repository (especially `n8n-templates/`) so student and organization work stays consistent with the NoéMI Architecture Blueprint.

This is **generic student guidance**. Use placeholder names such as `{your-org}-agents` for your own repository. Do not copy another organization's private project names or structure verbatim.
...
| You need pull requests, CI checks, an audit trail of every change, or easy rollback | It is a knowledge-base article for non-technical end users |
...
Keep a clear `README.md` in your GitHub repository that links to the relevant SharePoint site or folder. Technical, executable, and versioned artifacts live in GitHub. Human collaboration and non-code knowledge live in SharePoint.
...
## Suggested Starter Structure for a Student or Organization Repo

When you create your own repository, start with a simplified mirror of this blueprint. Name the repository after *your* organization or team — for example `{your-org}-agents` or `{your-team}-virtual-workforce`.
...
├─ integrations/                  # MCP or other integration experiments
├─ power-automate-workflows/      # Exported Solutions or unpacked flows
│   └─ README.md                   # How to import / environment notes
├─ agents/                        # Agent specs you create or adapt
├─ scripts/                       # Helper scripts
├─ .gitignore                     # Never commit secrets, .env, node_modules, etc.
└─ LICENSE (optional)
```

Look at the real structure of this repository (`n8n-templates/`, `agents/`, `mcp-protocols/`, `docs/examples/`) for production-grade patterns. Copy *patterns*, not another customer's project names or private contents.
...
Microsoft is adding native GitHub as a Git provider for Power Platform Solutions source control. Until that is generally available in your tenant, the export → commit process above is the reliable, tool-agnostic method.

### Desktop flows note

Power Automate Desktop has its own version-control features inside the product. For cloud flows that integrate with SharePoint, Teams, Dataverse, and similar services, the Solution export method is the standard path for GitHub.

---

## First Steps if You Are New to GitHub

1. Create a GitHub account (or use your existing one) and, if appropriate, join or create an organization for your company or team.
2. Create a new repository named for *your* organization or team (start private if preferred). Initialize with a README.
3. Either:
...
- Your personal or organization copy is where you customize and add *your* workflows and experiments.
...
- [ ] Created a GitHub repository named for your own organization or team
...
- [ ] Never committed secrets or connection strings (use `.env.template` + a secret manager)
- [ ] Did not copy another organization's private repo name, structure, or contents as a template
