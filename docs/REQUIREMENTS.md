...
- **Executive Assistant Test Framework Divergence**: `tools/executive-assistant/` uses `vitest` and `supertest` (ESM, `"type": "module"`) for its test suite, while the repository mandates the built-in Node test runner (`node --test`) as the primary validation framework (Requirement §9). The tool's tests under `tools/executive-assistant/tests/` are not executed by the canonical `npm run validate` gate (root `npm test` globs only `tests/*.test.js`), leaving the tool's triage and OAuth logic outside CI coverage.
- **Undocumented Executive Assistant Reference Service**: `tools/executive-assistant/server.js` implements a full Gmail Cloud Run Pub/Sub triage service — an admin REST API (`/api/queue`, `/api/resolution`, `/api/rules`, `/api/stats`, `/api/logs`), a confidence-scored human-in-the-loop review queue, and a "Learning Agent" resolution loop — but is not documented as a reference example in Requirement §8 and has no linked agent persona in the Agent Index. The codebase implements more than the requirements describe.
- **Committed Coverage Artifacts**: Generated coverage reports under `tools/executive-assistant/coverage/` (20 tracked files, including `coverage-final.json` and `clover.xml`) are committed to source control, and `.gitignore` lacks a `coverage/` exclusion (it covers `test-artifacts/` but not generated coverage output), drifting from clean-repository hygiene.
