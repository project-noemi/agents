...
- **Fetch-on-Demand**: When writing code that requires configuration, always assume the values will be provided via process memory environment variables (e.g., `os.getenv()`). Do not create local `.env` parsing logic. Use `.env.template` as the canonical reference for SecretOps command wrapper documentation.
- **Framework Injection**: The context generation scripts inject the framework layer (Value Lenses and Operating Profiles) into the generated context. Agents should rely on these injected definitions for context adaptation and success criteria.
