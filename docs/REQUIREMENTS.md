...
- **Golden Fixture Coverage Gap (Value Lenses & Operating Profiles)**: Decision [2026-05-28-0005] mandated injection of `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS` into generated context files, and `generate_all.js` now implements both. However, `tests/golden-fixtures.test.js` and `scripts/update-golden-fixtures.js` only cover 4 of the 6 template injection markers — the two new framework sections are excluded from the golden fixture regression safety net. A content regression in value lenses or operating profiles would not be detected by the test suite.
- **Test Harness Skills Coverage Asymmetry**: `tests/contracts.test.js` validates agent persona contracts but has no equivalent test for skill contracts. The `REQUIRED_SKILL_SECTIONS` check is enforced only by `scripts/audit-repo.js`. A developer running `npm test` alone cannot detect skill contract violations; the full `npm run validate` (audit + test) is required. This asymmetry is undocumented and creates false confidence on the test-only path.
- **Gatekeeper Config Schema Absent**: The Gatekeeper persona (`agents/engineering/gatekeeper.md`) references `.gatekeeper/config.yml` for per-repo overrides (allowlisted file patterns, custom thresholds). Decision [2026-05-28-0002] bootstrapped the `.gatekeeper/` directory with a `.gitignore` placeholder, but no YAML schema, documented field reference, or `config.yml.example` template has been defined. The persona is operationally configurable in specification but undocumented in structure.
- **Case-Sensitivity Mismatch Between Test Suite and Audit Script**: Decision [2026-05-28-0004] implemented case-insensitive heading comparison in `scripts/audit-repo.js`, but `tests/contracts.test.js` still uses strict case-sensitive string equality for persona contract heading checks. A persona file with non-canonical capitalization (e.g., `## refusal criteria`) can pass the audit gate while failing the standard test suite, creating a contradictory and confusing developer experience.
