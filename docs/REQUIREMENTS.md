...
- **SecretOps Authentication Enforcement Gap**: `scripts/verify-env.sh` (lines 140-164) and `scripts/verify-env.ps1` (lines 99-125) currently issue only warnings for missing or invalid SecretOps authentication (e.g., failing `infisical whoami` or `op user get --me`) even in `docker` mode, violating the mandate for a fatal error (exit 1) in production-like environments.
...
- **SecretOps Provider Bias**: `tests/examples-smoke.test.js` contains hardcoded assertions for the `op://` (1Password) pattern (e.g., `assert.match(content, /op:\/\//)` at line 20), causing false failures for users of Infisical or other providers.
- **Automation Branch Flow Undocumented Under the develop-only Merge Gate**: Decision [2026-07-03-0001] makes `develop` the only valid PR source into `main`. Governed automation runs on `claude/*` session branches and `sync/upstream-*` branches whose PRs must therefore target `develop`, but Requirement §7 and the automation prompt docs (`docs/DEV_AGENT_PROMPT.md`, `docs/SYNC_AGENT_PROMPT.md`) do not yet document this integration path.
- **Internal Tool Observability Gap**: While the shared `scripts/audit_logger.js` utility is available, Node.js tools like `executive-assistant` (`tools/executive-assistant/server.js`, specifically lines 19, 44, 49, 103, 107, 121, 125, 150, 156, 161) and reference services like `dashboard-ingest.js` still rely on unstructured `console.log` and have not yet adopted the mandated JSON Audit Log emission to `stderr`. This includes events like `[Pub/Sub] Notification received` and `[Triage Router] Execution Complete`.
...
- **Sub-Tool Testing Framework Drift**: `tools/executive-assistant/package.json` utilizes `vitest` for tests, representing a testing framework drift from the root-level native Node.js test runner.
...
- **Phase 0 Audit Gap**: `scripts/audit-repo.js` lacks logic to verify the presence of exactly 9 mandated assessment files in `docs/phase-zero-assessment/` (e.g., `network-security-assessment.md`, `readiness-rubric.md`, `PRACTITIONER_NOTES.md`), violating Requirement §1.
- **Agent Index Descriptive Truncation (Sentence Regex Failure)**: `scripts/context_helpers.js` (line 190) extracts only the first sentence of the `Role` section using the regex `/^[^.!?]+[.!?]/`, violating the "paragraph extraction" mandate in `AGENTS.md`. This regex fails to capture full technology names containing dots (e.g., "Next.js" is truncated to "Next."), leading to inaccurate role summaries in the Agent Index.
- **Generator "Silent Success" on Missing Framework Assets**: `scripts/generate_all.js` (lines 53-73) (via `context_helpers.js` lines 265, 277) returns HTML comments (e.g., `<!-- Framework directory not found... -->`) when `value-lenses/` or `operating-profiles/` are missing or empty, rather than failing the build. This violates the "Fail Fast" mandate for generator drift (Requirement 3).
...
- **Rules & Constraints Heading Naming Inconsistency**: 90% of agent personas (e.g., `linux.md`, `student-success-coach.md`) use `## Rules & Constraints (4D Diligence)` or similar headings, creating a documentation drift from the `AGENTS.md` mandate for `## Rules & Constraints`.
...
- **Undocumented Admin Control Surface**: `tools/executive-assistant/server.js` (lines 19, 44, 49, 103, 107, 121, 125, 150, 156, 161) implements several admin-facing API endpoints (`/api/queue`, `/api/stats`, `/api/logs`, `/api/rules`, `/api/resolution`) that provide unauthenticated access to internal agent state, queued tasks, and rule configurations, creating a governance drift from the security baseline. The `/admin` path serves a Vite-based UI from `ui/dist`.

### 3. Substantive & Safety Drift
- **Substantive Content Baseline (The "TBD" Problem)**: 100% of reusable skills (e.g., `skills/reporting/alert-notify.md`) and several framework assets (e.g., `value-lenses/care-continuity.md`) use "TBD" placeholders or boilerplate for `Data Inventory` and `Refusal Criteria`, bypassing the D2 (Description) and Refusal Principle mandates.
...
- **Infrastructure Asset Implementation Gap** (Remediated: 2026-07-10, verified 2026-07-10): `templates/tiers/basic.md`, `premium.md`, and `standard.md` now exist, along with `operating-profiles/standard-operating-profile.md`.
