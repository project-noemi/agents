...
## [2026-06-18-0001] Referential Integrity Audit for `scripts/audit-repo.js`

- **Decision:** `scripts/audit-repo.js` MUST acquire a referential-integrity pass that validates internal cross-references across the documentation set. Concretely, the audit must verify: (1) every entry in `mcp.config.json` `active_mcps` and `active_skills` lists maps to a file that exists under `mcp-protocols/` and `skills/` respectively; (2) every `**Skill:** [path]` reference in any agent persona under `agents/` points to a real `skills/*.md` file; (3) every internal markdown link (`[text](path/to/file.md)`) in `docs/`, `agents/`, `skills/`, and root-level mandate files resolves to a tracked file in the repository. Failures are emitted as fatal audit errors in CI and as a non-fatal warning bundle in local runs to keep contributor friction low.
- **Context:** Resolves CLARIFICATIONS.md Q [2026-05-02] (Config-to-Asset Mapping Validation), Q [2026-05-15] (Skill-to-Agent Referential Integrity), and Q 2026-06-17 (Automated Internal Documentation Link Integrity). All three are the same drift: structural audits verify headings but not the targets those headings cite, so file renames (e.g., `n8n workflows` → `n8n-workflows`) silently strand references. Internal link validation is an industry-standard guard for documentation-heavy repositories and a natural extension of the audit-as-gate principle already in place.
- **Impact:** Closes the `Audit Script Enforcement Depth` and `Internal Referential Integrity Gap` items in `REQUIREMENTS.md` Current Known Limitations once shipped. The check lands as part of the existing `scripts/audit-repo.js` invocation so `npm run validate` continues to be the single canonical gate. Implementation is tracked separately as `REQ-AUDIT-REFINT` in `REQUIREMENTS.md`.

## [2026-06-18-0002] Internal Tool Audit Log Event-to-Schema Mapping

- **Decision:** Internal Node.js tools in `tools/` and reference services in `examples/` that fall under the audit-log mandate MUST adopt the following event-to-schema mapping when emitting structured JSON Audit Logs to `stderr`:
  - `task` — a short, slug-style identifier for the tool-internal event (e.g., `executive-assistant.triage-vip`, `executive-assistant.sync-complete`, `dashboard-ingest.report-accepted`, `dashboard-ingest.signature-rejected`). Tools may namespace `task` with `<tool>.<event>` to disambiguate.
  - `inputs` — the operational inputs that triggered the event, excluding secrets and PII (e.g., message id, recipient hash, request path, payload size). For receive-side services this is the sanitized request envelope; for triage tools this is the source identifier.
  - `actions` — the discrete steps the tool actually executed in response (e.g., `["validated-hmac", "appended-to-influx", "emitted-ack"]`). One-entry arrays are valid.
  - `risks` — any risks identified or rejected during the event (e.g., `["missing-signature"]`, `["payload-too-large"]`, `[]` when none).
  - `result` — a terminal outcome string (`success`, `rejected:<reason>`, `error:<reason>`). The free-form suffix is allowed but the leading token is constrained to a small vocabulary so consumers can route on it.
  - The shape remains identical to the canonical agent-persona Audit Log; only the semantics of `task` are tool-specific. No new schema is introduced — the existing `{ "task", "inputs", "actions", "risks", "result" }` contract is reused verbatim. Tools MUST use the shared `scripts/audit_logger.js` utility (Decision [2026-05-28-0001] envelope + this mapping) once that utility lands; until then they MAY emit the JSON directly to `stderr`.
- **Context:** Resolves CLARIFICATIONS.md Q [2026-06-15] (Internal Tool Audit Log Event Mapping) and Q [2026-06-17] (Internal Tool Audit Log Mapping). The mandate in `AGENTS.md` to emit JSON Audit Logs is unimplementable today because contributors have no canonical mapping between the existing `execLogs` events used inside `tools/executive-assistant/` (`event`, `details`, `actions`) and the canonical 5-key schema. A standardized event-to-schema mapping is a prerequisite for the existing mandate to be actionable; defining it does not introduce a new contract, it makes the existing one runnable.
- **Impact:** Closes the documentation half of the `Internal Tool Observability Gap` in `REQUIREMENTS.md` Current Known Limitations. The implementation half (refactor of `tools/executive-assistant/` and `examples/gatekeeper-deployment/dashboard-ingest.js`) is tracked separately as `REQ-INTERNAL-AUDIT-LOG-IMPL` in `REQUIREMENTS.md`. The shared `scripts/audit_logger.js` utility remains the canonical sink once available.

## [2026-06-18-0003] Substantive Persona Content Remediation — Deferred Pending Product-Owner Authorization

- **Decision:** A fleet-wide bulk replacement of the boilerplate `Refusal Criteria` and `Data Inventory` content across all 22+ agent personas in `agents/` is **deferred** pending explicit product-owner authorization. The repository continues to track the substantive drift as an open limitation; structural compliance gates remain in place; no autonomous bulk content rewrite is performed in this cycle.
- **Context:** Resolves CLARIFICATIONS.md Q [2026-04-26] (Substantive Persona Content Drift), Q [2026-05-10] (Substantive Persona Remediation Strategy), and Q [2026-06-17] (Substantive Content Baseline for Agents) — all three ask the same question. A fleet-wide content rewrite touches every persona, materially changes refusal semantics for production agents, and cannot be confidently performed by an automated cycle without role-specific domain input from the Explorer (Passenger) for each agent. Performing it autonomously risks generating plausible-sounding but operationally wrong refusal criteria, which is more dangerous than a known-boilerplate baseline.
- **Impact:** `REQUIREMENTS.md` keeps `Structural vs. Substantive Compliance` and `Substantive Content Boilerplate` listed as Current Known Limitations. When the product owner authorizes, the work proceeds agent-by-agent (per the small-chunks-close-loops policy) rather than as one batched PR.
