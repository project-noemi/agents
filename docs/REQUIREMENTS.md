...
- **Compassion Value Lens Markdown Specification Asset Gap**: Although `value-lenses/compassion-lens.json` is present and `value-lenses/README.md` references the "Project NoéMI Anti-Replacement rule," the companion `value-lenses/compassion-lens.md` file is missing. The markdown-based context generator and audit scripts are thus blind to its specific Success Criteria, Preferred Evidence, and Tradeoff logic.
- **Casdoor Verification Reference Asset and Middleware Gap**: Decision [2026-06-12-0011] mandates `skills/security/casdoor-validate.md` to document JWT validation and an optional token-validation middleware sample in `examples/gatekeeper-deployment/dashboard-ingest.js` (controlled by `CASDOOR_VALIDATE_TOKENS=true`). However, neither the markdown asset nor the middleware sample exists in the repository.
