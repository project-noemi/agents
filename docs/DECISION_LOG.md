...
## [2026-07-09-0001] PR #273 Reconciliation — Sync-Down Merge, Duplicate Closures, Strict Gate Restored

- **Decision:** The direct-to-main doc-uplift PR #273 (`doc-requirements-uplift-2026-07-06-*`) is reconciled onto `develop` via a sync-down merge rather than reverted. Its three re-added clarifications are closed as duplicates of already-decided items: "Automation Branch Flow" (decided by [2026-07-07-0002]), "Audit Script TBD Placeholder Rejection" (decided by the [2026-05-20]/[2026-06-19-0005] audit-coverage family), and "Agent Index Regex Refinement" (decided by [2026-06-19-0012]). Its Known-Limitations rewrites are superseded by develop's strictly newer entries (the [2026-07-05-0005..0012] and [2026-07-07-0001..0004] series). The `require-develop-source` gate is resolved to the decided strict state — `develop` as the only valid PR head into `main` — because the `^doc-requirements-uplift-` wildcard re-added on `main` by commit `a0a7fad` (`google-labs-jules[bot]`, 2026-07-06, "Fix CI deadlock") has no decision entry and directly contradicts [2026-07-03-0001] and Requirement §7. The wildcard regression itself is NOT silently ratified or silently reverted on `main`: it is recorded as a Current Known Limitation and raised as Clarification [2026-07-09] Merge Gate Wildcard Regression, because removing it operationally breaks the still-misconfigured Jules doc-uplift routine until its scheduler is repointed at `develop` — a change outside this repository.
- **Context:** PR #273 was generated from `main` by the Jules routine two days after [2026-07-07-0002] codified that automation must target `develop`. It could only merge because `a0a7fad` re-widened the gate minutes earlier — the second live demonstration of the automation-branch-flow failure mode after PR #266 (see [2026-07-05-0012]). The result was renewed `main`/`develop` divergence: `main` held four commits `develop` lacked, and both lineages edited `docs/REQUIREMENTS.md` and `docs/CLARIFICATIONS.md`, producing merge conflicts in this sync-down.
- **Impact:** The develop-superset invariant is restored by this branch's merge commit. CLARIFICATIONS.md does not regain the three duplicate questions. The strict gate ships to `main` with the next `develop → main` release PR; until the PO answers the repointing clarification, the next Jules run against `main` will fail the gate by design.

## [2026-07-09-0002] Service Tier Templates and Standard Operating Profile — Shipped; Clarifications Closed as Overtaken

- **Decision:** Q [2026-06-10] Empty Tier Templates Implementation Gap and Q [2026-06-15] Service Tier Template Specifications are closed as overtaken by events: `templates/tiers/basic.md`, `standard.md`, and `premium.md` now exist with substantive content (positioning, provisioned MCPs, limits — no TBD placeholders), shipped by commit `9c19548` ("salvage gap-filling assets from stale PR #254") together with `operating-profiles/standard-operating-profile.md`. Decision [2026-06-19-0009] (tier templates deferred pending PO-supplied commercial definitions) is annotated as overtaken by this shipped implementation; if the PO later supplies authoritative commercial definitions, they amend the shipped starter templates rather than reopen the question.
- **Context:** Verified in the current tree: all three tier templates and the standard operating profile exist and contain no placeholder text; the generated `CLAUDE.md`/`GEMINI.md` operating-profile injection block is no longer empty. Of the three-profile starter set approved by [2026-06-19-0011], only `standard-operating-profile.md` has shipped — `high-trust-regulated.md` and `fast-iteration-startup.md` remain unimplemented.
- **Impact:** CLARIFICATIONS.md shrinks by two questions. The `REQUIREMENTS.md` "Infrastructure Asset Implementation Gap" limitation is narrowed to the two missing operating profiles; the shipped assets move to the Remediated archive.
