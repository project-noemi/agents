...
- **Agent Index Accuracy Drift** (Remediated: 2026-06-29): `scripts/context_helpers.js` now extracts the full first paragraph of the `Role` section for the Agent Index table (Decision [2026-06-29-0001]). Newline-collapsed and length-capped at 400 chars for readability.
...
- **Audit Script Coverage Expansion (Partial, 2026-06-29)**: Decision [2026-06-29-0001] landed four of the six checks in `scripts/audit-repo.js` as warning-only supplementary audits: Node 24 baseline (Dockerfile / docker-compose.yml), AI model baseline (`gemini-2.5-flash` enforcement in `examples/` and `tests/`), naming-convention regex (slug-based filenames across `docs/`, `agents/`, `skills/`, `examples/`, `tools/`), and mcp.config.json referential integrity (active_mcps / active_skills must point to real files). Still open: Refusal Criteria substantive check and skill-to-agent reference integrity. Warnings will be promoted to failures once underlying drifts are remediated.
...
- **Gatekeeper Mutating Actions Pending**: Decision [2026-06-12-0005] mandates a dry-run mutating mode in `examples/gatekeeper-deployment/` with a `GATEKEEPER_MUTATING_MODE` switch. Pending implementation.
- **Casdoor Reference Skill Pending**: Decision [2026-06-12-0011] mandates a `skills/security/casdoor-validate.md` and an optional middleware sample in `dashboard-ingest.js`. Pending implementation.
- **Agent Index First-Paragraph Extraction** (Remediated: 2026-06-29): Closed by Decision [2026-06-29-0001]. `scripts/context_helpers.js` now extracts the full first paragraph of the `Role` section (Decision [2026-06-12-0012] / [2026-06-19-0012]).
- **RFP Split Naming Remediation** (Remediated: 2026-06-12): Files in `examples/rfp-split/` use slug-based naming per Decision [2026-06-12-0013].
- **Generator Script Consolidation** (Remediated: 2026-06-12): `scripts/generate_gemini.js` and `scripts/generate_claude.js` are now thin shims that delegate to `scripts/generate_all.js` per Decision [2026-06-12-0015] / [2026-06-19-0012].
- **AI Model Baseline Drift in Legacy Example** (Remediated: 2026-06-29): `examples/docker/agent.py` was updated from `gemini-2.0-flash` to the canonical `gemini-2.5-flash` baseline. The supplementary AI-model audit in `scripts/audit-repo.js` (Decision [2026-06-29-0001]) now flags any future regression.
- **Internal Tool / Build Utility Audit Log Pending**: Decision [2026-06-12-0010] mandates JSON Audit Log to stderr from build utilities, internal tools, and reference services. Pending implementation; closes the `Audit Log Emission Gaps` and `Internal Tool Observability Gap` items when shipped.
