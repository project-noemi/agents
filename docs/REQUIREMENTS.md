...
- **Audit Script Enforcement Depth** (Partially Remediated: 2026-06-01): Skills heading compliance audit is now implemented in `scripts/audit-repo.js` via `checkSkills()` per Decision [2026-05-20]. Remaining open gaps: (a) referential integrity — the audit does not verify that MCP keys in `mcp.config.json` or skill references in agent `Workflow` sections map to existing files; (b) own JSON Audit Log emission — Decision [2026-05-20] mandated that `audit-repo.js` itself emit a structured JSON Audit Log to `stderr` upon completion; this is not yet implemented.
...
- **Internal Tool Observability Gap**: Node.js tools in `tools/` (e.g., `executive-assistant`) and reference services in `examples/` (e.g., `dashboard-ingest.js`) lack structured JSON Audit Log emission to `stderr`, relying on unstructured `console.log` instead.
...
- **Golden Fixture Coverage Gap** (Identified: 2026-06-01): Decision [2026-05-28-0005] added `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS` as active injection markers in `scripts/generate_all.js`. However, `tests/golden-fixtures.test.js` and `scripts/update-golden-fixtures.js` were never updated to include these two markers. Changes to `value-lenses/` or `operating-profiles/` content will not be caught by the golden fixture regression suite, creating a silent regression path for framework-layer content.
- **Nested Agent Directory Pattern Undocumented** (Identified: 2026-06-01): Five coding-domain agents use a three-level directory structure (`agents/coding/{type}/core.md`) that deviates from the two-level convention (`agents/{domain}/{name}.md`) documented in `AGENTS.md` and `CLAUDE.md` under "Adding or Modifying Agents." The `discoverAgents()` function handles the nesting transparently, but contributor-facing documentation has not been updated to acknowledge or authorize this pattern, creating onboarding friction for new contributors in the `coding/` domain.
- **Dashboard Ingest Route Not Directly Tested** (Identified: 2026-06-01): `tests/examples-smoke.test.js` verifies that the Docker Compose env var `DASHBOARD_API_URL` references `/api/v1/reports`, but does not directly assert on the route handler in `examples/gatekeeper-deployment/dashboard-ingest.js`. A route change in the Node.js file would not be caught by the current smoke test suite.
