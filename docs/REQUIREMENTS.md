...
- **Audit Script Enforcement Depth** (Remediated: 2026-06-05): `scripts/audit-repo.js` now verifies physical existence of skills and MCPs from `mcp.config.json`, performs JSON schema validation for Audit Logs, and detects "TBD" placeholders.
- **Branch Protection Audit Gap** (Remediated: 2026-06-05): `scripts/audit-repo.js` now includes a status check for branch protection setup.
- **Agent Index Accuracy Drift** (Remediated: 2026-06-05): `scripts/context_helpers.js` now extracts the full first paragraph of the `Role` section for the Agent Index.
...
- **Internal Tool Observability Gap**: Node.js tools in `tools/` (e.g., `executive-assistant`) and reference services in `examples/` (e.g., `dashboard-ingest.js`) lack structured JSON Audit Log emission to `stderr`, though the shared `audit_logger.js` utility is now available.
...
- **Sync Script Parameterization Gap** (Remediated: 2026-06-05): `scripts/sync-upstream.sh` now uses environment variables (`NOEMI_UPSTREAM_URL`, `NOEMI_ORG_NAME`) for parameterization.
