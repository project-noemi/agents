...
### ❓ Question [2026-06-05] - Context Generation Injection Set Drift (§4)
**Context:** `docs/REQUIREMENTS.md` §4 ("Context Generation Must Stay Aligned") states the generators "must inject: the full mandate set from `AGENTS.md`; the agent index discovered from `agents/`; active skills from `mcp.config.json`; active MCP protocol content from `mcp.config.json`" — four payloads. However, `scripts/generate_all.js` (lines 50–73) injects six payloads, additionally rendering `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS` via `buildFrameworkSection`. Both markers are enforced by `REQUIRED_TEMPLATE_MARKERS` in `scripts/context_helpers.js` (lines 25–32) and asserted by the "context templates retain all required injection markers" test in `tests/contracts.test.js`.
**Ambiguity / Drift:** The requirement under-describes the implemented, audited, and golden-fixture-tested generation contract. The "Framework Injection Gap (Remediated: 2026-05-28)" note in Known Limitations confirms framework injection now ships, but §4's authoritative injection list was never updated to match. A maintainer treating §4 as the implementation truth could remove the Value Lens / Operating Profile injection believing it non-canonical, breaking the audit and golden-fixture suites.
**Question for Product Owner:** Should §4's injection list be updated to formally include the Value Lens and Operating Profile framework injections as mandatory generator outputs, matching the implemented and audited behavior?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Update `docs/REQUIREMENTS.md` §4 to add "the Value Lens framework sourced from `value-lenses/`" and "the Operating Profile framework sourced from `operating-profiles/`" to the mandatory generator injection list, and archive the rationale to `DECISION_LOG.md`.*

### ❓ Question [2026-06-05] - Audit Gate Failure Conditions Incomplete (§3)
**Context:** `docs/REQUIREMENTS.md` §3 ("Contract and Generator Drift Must Fail Fast") enumerates five conditions under which `scripts/audit-repo.js` must fail. In practice `audit-repo.js` enforces additional conditions that §3 does not list: a missing `### Refusal Criteria` H3 subsection under `## Rules & Constraints` (lines 75–79), an entirely missing `## Audit Log` section (line 100), an Audit Log present but missing the mandatory JSON shape (line 97), and a persona missing its `## Role` section required for the Agent Index (lines 110–113).
**Ambiguity / Drift:** §3 is the authoritative description of the audit gate, yet it silently omits the Refusal Criteria structural check — a safety-relevant gate tied directly to the Refusal Principle. This documentation drift risks a maintainer "simplifying" the audit to match §3 and inadvertently dropping Refusal Criteria enforcement.
**Question for Product Owner:** Should §3's failure-condition list be expanded to document the `### Refusal Criteria` subsection check, the missing/empty `## Audit Log` checks, and the missing `## Role` check that `audit-repo.js` already enforces?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Update `docs/REQUIREMENTS.md` §3 to enumerate the `### Refusal Criteria` subsection check, the missing `## Audit Log` section check, and the missing `## Role` check as mandatory audit failure conditions, and archive the change to `DECISION_LOG.md`.*

### ❓ Question [2026-06-05] - Undefined Canonical AGENTS.md Mandate Section Set (§3)
**Context:** `docs/REQUIREMENTS.md` §3 states the audit must fail when "`AGENTS.md` is missing required top-level mandate sections," and §2 references "required top-level mandate sections" abstractly. The concrete enforced list lives only in code: `REQUIRED_GLOBAL_SECTIONS` in `scripts/context_helpers.js` (lines 17–23) hardcodes the canonical five — `🔐 Secrets & Configuration`, `🛡 Error Handling and Resilience`, `🚀 Execution Patterns`, `🛠 Local Development & Authentication`, and `📝 Coding Standards`.
**Ambiguity / Drift:** The requirement uses the vague phrase "required top-level mandate sections" without enumerating them, so the source of truth for which sections are mandatory is the code, not the spec. This inverts the intended relationship (where `REQUIREMENTS.md` is the implementation truth) and makes the contract impossible to verify by reading the spec alone.
**Question for Product Owner:** Should §3 (or a referenced appendix) explicitly enumerate the five canonical mandatory `AGENTS.md` top-level sections so the requirement — not `context_helpers.js` — is the documented source of truth?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Add the canonical five mandatory `AGENTS.md` top-level mandate sections (matching `REQUIRED_GLOBAL_SECTIONS`) to `docs/REQUIREMENTS.md` §3, and add a contracts test asserting the spec list and `REQUIRED_GLOBAL_SECTIONS` stay in sync.*
