...
- **Skill Contract Substantive Drift**: 100% of reusable skills in `skills/` contain "TBD" placeholders for mandatory `Data Inventory` and `Refusal Criteria` sections, failing substantive compliance with the 4D framework.
- **Docker Smoke Test Variable Validation Gap**: `tests/examples-smoke.test.js` lacks validation for mandated `NOEMI_DOCKER_SMOKE_*` environment variables (Requirement 9).
- **Sync Script Parameterization Gap**: `scripts/sync-upstream.sh` relies on hardcoded organization placeholders instead of environment-based parameterization.
- **Golden Fixtures Framework Marker Gap**: `tests/golden-fixtures.test.js` covers four injection markers (`GLOBAL_MANDATES`, `AGENT_INDEX`, `SKILLS_INJECTIONS`, `MCP_INJECTIONS`) but lacks golden fixture files for `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS`, which were mandated by Decision [2026-05-28-0005]. `scripts/generate_all.js` and `scripts/context_helpers.js` support these markers, but injector correctness for Value Lens and Operating Profile sections is unverified by CI, creating a regression risk.
- **GMU Validation Example Governance Gap**: `examples/gmu-validation/verification-bot.js` implements a Node.js simulation of a student audit for a George Mason University Boot Camp scenario. This example has no README, no entry in REQUIREMENTS.md, and no governance decision in DECISION_LOG.md. Its documentation status, maintenance ownership, and relationship to the canonical deployment examples are undefined.
- **Test Coverage Specification Drift**: Requirement §9 describes the default test suite at a high level of abstraction. `tests/contracts.test.js` has grown to validate 14+ distinct artifact categories not enumerated in §9, including cross-platform kickstart guides (macOS/Linux, Windows, ChromeOS), Microsoft 365 agentic client docs, Google Workspace multi-path docs, agentic local workspace docs (Gemini, Claude, Codex), visual guides with mermaid diagrams, n8n anti-hallucination verification, RFP responder workflow Gemini node type, and Node.js baseline pinning across `.nvmrc` and `.node-version`. The requirement is under-specified relative to the implemented truth.
- **Audit Script Partial Remediation Note** (Partial — 2026-05-30): `scripts/audit-repo.js` now includes `checkSkills()` which validates skill contract headings and Audit Log JSON schema, partially satisfying Decision [2026-05-20] mandate. Remaining gaps: (1) the script does not emit its own structured JSON Audit Log to `stderr` as required by the same decision; (2) referential integrity checks (verifying skill references in agent Workflow sections) are still absent.
