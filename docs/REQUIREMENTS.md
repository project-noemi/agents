...
- **Sub-Package Node.js Baseline 'engines' Enforcement Gap**: Nested service and tool packages (such as `tools/executive-assistant/package.json`) do not configure the `"engines": { "node": ">=24" }` restriction, creating a risk of installing dependencies or executing runtime tasks on unsupported, older Node.js runtimes.
...
- **GMU Feynman Verification Bot Integration Gap**: The GMU verification script `examples/gmu-validation/verification-bot.js` implements the Capstone badge verification logic but is entirely undocumented in the requirements and skipped by the local test harness (`tests/examples-smoke.test.js`) and CI/CD validation gates.
- **Non-Markdown Value Lens JSON Asset Audit & Injection Gaps**: Value Lens definitions like `american-dream.json` and `compassion-lens.json` in `value-lenses/` are written in JSON and bypass the Markdown-only audit and context-generation scripts (`scripts/audit-repo.js`, `scripts/context_helpers.js`), which strictly check for and inject `.md` assets.
