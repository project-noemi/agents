...
- **Skill Contract Substantive Drift**: All 8 reusable skills now include the structural `## Data Inventory` and `### Refusal Criteria` headings (passing structural audits), but their content remains placeholder (`TBD` values) rather than role-specific definitions, failing the D2 Description standard. `SKILL_TEMPLATE.md` itself is still missing both `## Data Inventory` and `### Refusal Criteria` (it is excluded from the structural audit), meaning new skills scaffolded from the template start structurally non-compliant.
- **Audit Log JSON Schema Key Enforcement Gap**: `scripts/audit-repo.js` validates that the Audit Log section contains parseable JSON but does not verify the mandatory key set (`task`, `inputs`, `actions`, `risks`, `result`). The script also does not emit its own structured Audit Log to `stderr`. Both are explicitly mandated by Decision [2026-05-20] but remain unimplemented. Section 3 of this document states "Mandatory JSON shape validation" as if it were enforced — it is not.
