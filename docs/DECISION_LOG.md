...
## [2026-06-26-0001] Executive Assistant "Learning Agent" — Documented Pattern, Not a Distinct Persona

- **Decision:** The "Learning Agent" resolution path observed in `tools/executive-assistant/server.js` (`/api/resolution` endpoint feeding "historical messages passed" back into the triage router's heuristics) is recognized as a **feedback-loop pattern** of the existing Executive Assistant persona, NOT a separate agent persona. It will be documented in `REQUIREMENTS.md` under the Executive Assistant tool entry as a known pattern, and the audit script will not require a dedicated persona file. Industry practice for human-in-the-loop systems (e.g. Snorkel, active-learning routers) is to treat the feedback channel as a property of the parent classifier rather than as a separate actor; best practice for a reference architecture is to surface the pattern in documentation without proliferating persona files that would each carry placeholder safety sections; the recommendation is therefore documentation-only treatment.
- **Context:** Resolves the new clarification ported from PR #236 (main): Q [2026-06-20] Executive Assistant "Learning Agent" Resolution Path. Promoting it to a persona would force a 22→23 persona contract sprawl whose net new safety value is zero because the loop is bounded by the parent persona's existing Rules & Constraints and Refusal Criteria.
- **Impact:** Closes the clarification. Adds one short pattern note in `REQUIREMENTS.md`. No new agent file is created. The audit script remains unchanged.

## [2026-06-26-0002] Rules & Constraints Heading — Both Forms Accepted, Single-Source Audit

- **Decision:** The mandated heading remains **`## Rules & Constraints`** for agent personas and **`## Rules & Constraints (4D Diligence)`** for reusable skills. The two forms are NOT unified. Instead, `scripts/context_helpers.js` is the single source of truth for which heading each contract expects, and the audit script will continue to consult its `REQUIRED_AGENT_SECTIONS` / `REQUIRED_SKILL_SECTIONS` constants. Bulk-renaming 22+ personas would invalidate every existing golden fixture and every external orchestrator already grepping for the persona heading, with no proportional benefit. Industry practice for governance contracts is to keep the heading text load-bearing (because tools consume it) and to document the convention once; best practice is to make the difference a property of the contract definition rather than a property of every file; the recommendation is therefore status quo + this durable rationale.
- **Context:** Resolves the new clarification ported from PR #236 (main): Q [2026-06-20] Rules & Constraints Heading Inconsistency. The "inconsistency" is intentional — agents carry the 4D framework implicitly through their full nine-heading contract, while skills are smaller atomic units where the `(4D Diligence)` suffix makes the obligation explicit at the call site.
- **Impact:** Closes the clarification. No file rewrites. `audit-repo.js` continues to read the canonical lists from `scripts/context_helpers.js`.

## [2026-06-26-0003] Phase 0 Assessment Kit — Add Inventory Audit

- **Decision:** `scripts/audit-repo.js` SHALL verify the presence of the nine canonical files in `docs/phase-zero-assessment/` (`README.md`, `security-assessment.md`, `ai-readiness-assessment.md`, `network-security-assessment.md`, `PRACTITIONER_NOTES.md`, `consent-template.md`, `report-template.md`, `roadmap-template.md`, `readiness-rubric.md`). A missing file fails the audit. This protects the buyer entry point mandated by REQUIREMENTS.md §1. Industry practice for governance assets is to inventory them in the same gate that audits agent personas; best practice is to make the inventory list explicit in the audit script (not as a glob) so a deliberate rename is recorded as a code change; the recommendation is therefore an explicit allowlist with the current nine names.
- **Context:** Resolves the new clarification ported from PR #236 (main): Q [2026-06-20] Phase 0 Assessment Kit Audit Coverage. Implementation shipped in this PR via a new `checkPhaseZeroAssessmentKit()` function in `scripts/audit-repo.js`.
- **Impact:** Closes the clarification AND the underlying audit gap. Removes "Phase 0 Assessment Kit audit coverage" as an open task.

## [2026-06-26-0004] Resilience Helper Module System Mismatch — Resolve via ESM Shim

- **Decision:** A thin ESM bridge (`scripts/resilience_helpers.mjs`) and a matching `scripts/audit_logger.mjs` re-export the CommonJS implementations for ESM consumers (e.g. `tools/executive-assistant/`). This preserves the single-source CJS implementation that Node-stdlib tests already exercise while unblocking ESM tools that prefer named-import syntax. Industry practice for dual-format Node libraries is conditional exports in `package.json`; best practice for a script-only `scripts/` directory (no package boundary) is a sibling `.mjs` shim because there is no `package.json` to attach `exports` to; the recommendation is therefore the sibling-shim approach.
- **Context:** Resolves CLARIFICATIONS.md Q [2026-06-19] Resilience Helper Module System Mismatch and the ESM/CJS bullet appended to REQUIREMENTS.md in PR #236.
- **Impact:** Closes the clarification AND the "Resilience Helper Module System Mismatch" Current Known Limitations entry.

## [2026-06-26-0005] Gatekeeper Mutating Actions — Dry-Run Mode Implementation Shipped

- **Decision:** `examples/gatekeeper-deployment/entrypoint.sh` now honors the `GATEKEEPER_MUTATING_MODE` environment variable (default `false`). When enabled, classifications include `auto-merge` (mergeable + approved) and `close-stale` (conflicting). Execution is further gated by the existing `GATEKEEPER_DRY_RUN` (default `true`): in dry-run, the report records "Would auto-merge" / "Would close" counts; only when `GATEKEEPER_MUTATING_MODE=true` AND `GATEKEEPER_DRY_RUN=false` does the entrypoint invoke `gh pr merge` / `gh pr close`. The report and the JSON payload include both counters and the mode flags so dashboards can observe intent vs. execution.
- **Context:** Implements Decision [2026-06-12-0005]. The persona text already mandated mutating actions; the reference implementation had remained classification-only. Industry practice for autonomous PR bots (e.g. Mergify, Dependabot auto-merge) is a two-step opt-in (capability flag + dry-run flag); best practice for a reference architecture is to make both flags off-by-default so a fresh clone is observably safe; the recommendation is therefore the two-flag gate implemented here.
- **Impact:** Closes the "Gatekeeper Mutating Actions Pending" entry in REQUIREMENTS.md Current Known Limitations.

## [2026-06-26-0006] Casdoor Reference Skill — Spec Shipped

- **Decision:** `skills/security/casdoor-validate.md` now exists and follows the standard reusable-skill contract (`Purpose`, `Inputs`, `Procedure`, `Outputs`, `Data Inventory`, `Rules & Constraints (4D Diligence)` with `### Refusal Criteria`, `Boundaries`, `Audit Log`). The spec describes JWT validation against a Casdoor JWKS with strict claim verification, in-memory JWKS caching, and structured stderr audit emission. The matching middleware sample in `dashboard-ingest.js` remains gated on `CASDOOR_VALIDATE_TOKENS=true` and is therefore deferred to a follow-up PR — the skill specification is the prerequisite that has been missing.
- **Context:** Implements the skill-spec half of Decision [2026-06-12-0011]. The middleware sample half remains tracked as a residual implementation task.
- **Impact:** Closes the "Casdoor Reference Skill Pending" entry in REQUIREMENTS.md Current Known Limitations. Replaces it with a smaller "Casdoor middleware sample pending" entry.

## [2026-06-26-0007] Agent Index First-Paragraph Extraction — Implementation Shipped

- **Decision:** `scripts/context_helpers.js` now extracts the full first paragraph of each persona's `## Role` section (delimited by a blank line) for the Agent Index, replacing the prior first-sentence truncation. The role is preserved at its natural length; no character cap is applied. Industry practice for index-style summaries is one paragraph (not one sentence) because complex personas need at least two clauses to convey purpose-and-scope; best practice for a reference architecture is to make the rule the same across all personas; the recommendation is therefore "full first paragraph, no cap".
- **Context:** Implements Decision [2026-06-12-0012]. Generated `CLAUDE.md` and `GEMINI.md` are regenerated in this PR to reflect the new index text and the golden fixtures are updated accordingly.
- **Impact:** Closes the "Agent Index First-Paragraph Extraction Pending" entry AND the "Agent Index Accuracy Drift" / "Agent Index Descriptive Truncation" entries in REQUIREMENTS.md.

## [2026-06-26-0008] Dashboard Ingest Adopts audit_logger.js — Shipped

- **Decision:** `examples/gatekeeper-deployment/dashboard-ingest.js` now emits structured stderr NDJSON audit lines on every ingest attempt (success, auth rejection, signature rejection, malformed JSON, influx-write failure) via `scripts/audit_logger.js`. This is the first reference service to wire up the shared audit logger and is the template for follow-up adoption in `tools/executive-assistant/`.
- **Context:** Partial implementation of Decision [2026-06-12-0010] and [2026-06-19-0001]. Executive Assistant adoption remains open (ESM import path documented in [2026-06-26-0004]).
- **Impact:** Closes the dashboard-ingest portion of the "Audit Log Emission Gaps" Current Known Limitations entry. Executive Assistant adoption remains the next concrete step.
