...
### ❓ Question [2026-06-08] - Executive Assistant Test-Framework Drift
**Context:** Requirement §9 and the §"Runtime and Tooling Requirements" state that "the built-in Node test runner is the primary validation framework for repository contracts and smoke tests." The root suite honors this (`package.json` → `"test": "node --test --test-concurrency=1 tests/*.test.js"`). However, `tools/executive-assistant/package.json` declares `"type": "module"` and `"test": "vitest run"`, and its specs (e.g., `tools/executive-assistant/tests/triage-router.test.js`, `integration.test.js`) `import { describe, it, expect } from 'vitest'` and use `supertest`.
**Ambiguity / Drift:** The executive-assistant validates on a different framework (vitest/ESM) than the mandated built-in Node runner, and because the root gate globs only `tests/*.test.js`, the tool's triage-routing and OAuth logic never runs under `npm run validate`. The repository can pass its canonical validation contract while a shipped Node tool is entirely unverified by it.
**Question for Product Owner:** Should `tools/executive-assistant/` be migrated to the built-in Node test runner and folded into the root `npm run validate` gate, or is vitest an officially sanctioned exception for application-style tools in `tools/` (in which case Requirement §9 and `AGENTS.md` should explicitly carve out that exception)?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Either port `tools/executive-assistant/tests/` to the built-in `node:test` runner and add them to the root validation gate, or update Requirement §9 and `AGENTS.md` to formally sanction vitest for `tools/` and document how those suites are run in CI.*

### ❓ Question [2026-06-08] - NOEMI_DOCKER_SMOKE_* Default Value Mismatch
**Context:** Requirement §9 mandates "static smoke checks for example stacks and Docker env inventories (including `NOEMI_DOCKER_SMOKE_*` variable validation)," and `.env.template` is the canonical inventory. `.env.template` defines `NOEMI_DOCKER_SMOKE_TIMEOUT_MS=60000`, `NOEMI_DOCKER_SMOKE_POLL_INTERVAL_MS=5000`, and `NOEMI_DOCKER_SMOKE_ARTIFACT_DIR=./tests/e2e/artifacts`.
**Ambiguity / Drift:** The consuming code in `tests/e2e/docker-smoke.test.js` falls back to different hardcoded defaults when the variables are unset: `... || 600000` (10× the inventoried 60000), `... || 2000` (vs. 5000), and `... || 'test-artifacts/docker-smoke'` (vs. `./tests/e2e/artifacts`). A run without the env file uses timing and artifact paths that contradict the documented single source of truth, which can mask or distort e2e behavior and scatter artifacts.
**Question for Product Owner:** Which set of values is authoritative — the `.env.template` inventory or the in-code fallbacks — and should the two be reconciled so the code defaults exactly match the inventoried values?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Reconcile `NOEMI_DOCKER_SMOKE_*` defaults so `tests/e2e/docker-smoke.test.js` fallbacks match `.env.template` exactly (timeout, poll interval, artifact dir), and add a smoke assertion that the two sources agree.*

### ❓ Question [2026-06-08] - Audit Script Self-Observability Not Implemented
**Context:** Decision [2026-05-20] (Audit Script Coverage Expansion) explicitly required `scripts/audit-repo.js` to "(3) emit its own JSON Audit Log to `stderr` per the observability mandate," alongside the skills-coverage and JSON-parse work. The skills coverage and `JSON.parse` validation shipped, but a filesystem check confirms `audit-repo.js` (and `generate_all.js`) emit only unstructured `console.log` / `console.error`.
**Ambiguity / Drift:** A durable, already-resolved decision is contradicted by the implementation. This differs from the still-open "should build utilities log?" questions ([2026-05-02], [2026-05-19]) because clause (3) already answered "yes" for `audit-repo.js` specifically — the gap is implementation follow-through, not an undecided policy.
**Question for Product Owner:** Should Jules implement Decision [2026-05-20] clause (3) now (structured JSON Audit Log to `stderr` from `audit-repo.js`, and by extension `generate_all.js`), or was that clause intentionally descoped and should the decision be amended to reflect that?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Implement structured JSON Audit Log emission to `stderr` in `scripts/audit-repo.js` and `scripts/generate_all.js` per Decision [2026-05-20] clause (3), using the canonical `{ "task", "inputs", "actions", "risks", "result" }` shape.*
