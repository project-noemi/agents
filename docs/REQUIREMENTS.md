...
- **CI/CD "False Green" Risk in E2E**: `tests/e2e/docker-smoke.test.js` silently skips runtime checks if Docker is absent. Decision [2026-07-05-0003] approved an opt-in `FORCE_DOCKER_SMOKE=true` mandatory-mode flag; implementation pending — the test currently has no FORCE_DOCKER_SMOKE branch.
- ~~**SecretOps Provider Bias**~~ (Remediated: 2026-07-05 per Decision [2026-07-05-0004]): `tests/examples-smoke.test.js` now accepts either `op://` or `infisical://` vault-reference patterns and either `op run` or `infisical run` Compose wrappers, satisfying the multi-provider mandate in Requirement §5.
...
- ~~**AI Model Baseline Drift**~~ (Remediated: 2026-07-05 per Decision [2026-07-05-0005]): `examples/docker/agent.py` now uses `gemini-2.5-flash`, matching the canonical AI Model Baseline.
...
- **Phase 0 Audit Gap**: `scripts/audit-repo.js` lacks logic to verify the presence of the 8+ mandated assessment files in `docs/phase-zero-assessment/`, violating Requirement §1. Decision [2026-07-05-0008] approved adding a file-presence check that fails the audit when any Phase 0 kit file is missing; implementation pending.
...
- **Generator "Silent Success" on Missing Framework Assets**: `scripts/generate_all.js` returns HTML comments (e.g., `<!-- Framework directory not found... -->`) when `value-lenses/` or `operating-profiles/` are missing or empty, rather than failing the build. This violates the "Fail Fast" mandate for generator drift (Requirement 3). Decision [2026-07-05-0006] approved fail-fast for both missing framework assets and missing active skills/MCPs; implementation pending.
