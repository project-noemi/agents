...
- **Internal Tool Observability Gap**: While the shared `scripts/audit_logger.js` utility is available, Node.js tools like `executive-assistant` (`tools/executive-assistant/server.js`, lines 44, 49, 121, 161) and reference services like `dashboard-ingest.js` still rely on unstructured `console.log` and have not yet adopted the mandated JSON Audit Log emission to `stderr`. This includes events like `[Pub/Sub] Notification received` and `[Triage Router] Execution Complete`.
...
- **Phase 0 Audit Gap**: `scripts/audit-repo.js` lacks logic to verify the presence of the 10 mandated assessment files in `docs/phase-zero-assessment/` (e.g., `network-security-assessment.md`, `readiness-rubric.md`, `PRACTITIONER_NOTES.md`), violating Requirement §1.
