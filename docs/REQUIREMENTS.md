...
- **Merge-Gate Wildcard Re-Accretion and Automation Gate Self-Modification** (Answered 2026-07-10-0002; escalated 2026-07-14): Requirement §7 mandates that `develop` is the only valid PR source into `main` and that any hotfix exception must be "a single explicit entry... never a broad wildcard," but `.github/workflows/require-develop-source.yml` re-accreted two wildcard exceptions (`doc-requirements-uplift-*` and `doc-requirements-refinement-*`) via direct-to-main automation commits `a0a7fad` (2026-07-07) and `e54275a` (2026-07-09) — after Decision [2026-07-03-0001] removed the previous six. Verified 2026-07-14: the two wildcards were present on **both** `main`'s and `develop`'s copies of the gate (the 2026-07-13 reconciliation merge carried them onto `develop`); `develop`'s copy is stripped back to the bare develop-only check as of Decision [2026-07-14-0001], while `main`'s copy still carries them until the next release PR. **Escalation evidence (updated 2026-07-21):** the direct-to-main automation ran again on 2026-07-12 (PR #286, merged into `main` through the `doc-requirements-uplift-*` wildcard) and has since fired **nightly**: #291 (07-14) and #294 (07-15) were closed unmerged by hand, but #295 (07-16), #297 (07-17), #299 (07-18), #301 (07-19), and #304 (07-20) remain **open**, all targeting `main`. Each run carries a commit that edits the gate so its own PR can pass; the branch naming has churned across three schemes (`doc-requirements-drift-uplift-20260713-*`, `doc-requirements-drift-uplift-2026-07-13-*`, `jules-*`), and the newest gate edits (diff-verified 2026-07-21 on #301 and #304) add both a `^doc-requirements-drift-uplift-` wildcard and a **blanket `^jules-` wildcard** that would permanently allowlist every future Jules branch into `main`. Manual per-PR closure stopped after 2026-07-15 and demonstrably cannot converge against nightly scheme churn — the effective remediation is reconfiguring or disabling the task at its source (jules.google.com task list, runs as user WSwarm; task IDs are preserved in each PR body). Remaining sub-tasks: **(i)** author the `develop → main` release PR (carries the stripped gate to `main`; later timeline from `develop` wins on governance docs); **(ii — urgent)** close PRs #295/#297/#299/#301/#304 unmerged and reconfigure/disable the external Jules doc-uplift task so future runs base off and target `develop`; **(iii)** enable the weekly scheduled `develop → main` release routine; **(iv)** consider an audit guard and CODEOWNERS protection on the gate file (see Clarifications [2026-07-14]).

- **Governed Doc-Automation Review Pipeline Stall** (verified 2026-07-21): the scheduled Doc routine has followed Requirement §7 correctly since 2026-07-15 — but its output no longer lands. Six consecutive daily runs each opened a draft PR into `develop` (#293, #296, #298, #300, #302, #303; 2026-07-15 → 2026-07-20) and none has been reviewed or merged; `develop` has been frozen at commit `23bc278` since 2026-07-15 and `main` at `9a2a8cd` since 2026-07-12. Consequences already visible: the stacked PRs hold overlapping edits to the same Known Limitation entries (guaranteed merge conflicts), later runs independently re-discover drifts earlier unmerged runs already recorded (e.g., the committed `tools/executive-assistant/coverage/` artifacts appear in both #296 and #303), four decision-backed implementations sit unshipped in #296 (`FORCE_DOCKER_SMOKE` mode, Phase 0 kit audit check, five-key Audit Log schema validation, `tools/roi/baseline-config.json`), and the weekly `develop → main` release routine (Decision [2026-07-10-0002] sub-tasks (i)/(iii)) has not produced its first release PR — leaving the wildcard-carrying gate copy live on `main`. Until review resumes or the routine's output policy changes (see Clarification [2026-07-21] Doc-Automation Review Cadence), each additional scheduled run adds review debt faster than it adds documentation value.
