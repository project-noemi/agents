...
- **Machine-Identity PR Authorship (Mandatory)**: Agent-initiated pull requests MUST be authored by the `noemi-agent` machine identity, never with a human's credentials — the token that opens the PR determines authorship, and GitHub blocks a PR's author from approving it, so a human-authored agent PR collapses the human-reviews-AI gate. Use `bash scripts/agent-gh.sh pr create ...` where `gh` is available, or `node scripts/agent-pr.js` in remote sessions without it; both enforce the `AGENT_GH_EXPECTED_LOGIN` identity pin. If no machine-identity token resolves, stop and surface the gap — never fall back to human credentials. Approving and merging remain human-only acts (Decision [2026-08-03-0002]; canonical runbook `docs/MACHINE_IDENTITY.md`).
