...
## [2026-07-07-0001] Tier Template Clarifications Closed as Overtaken by Events

- **Decision:** Clarifications Q [2026-06-10] (Empty Tier Templates Implementation Gap) and Q [2026-06-15] (Service Tier Template Specifications) are closed without a product-owner answer because the codebase change they requested already shipped: commit `9c19548` ("salvage gap-filling assets from stale PR #254", merged 2026-07-05 via PR #264) added substantive `basic.md`, `standard.md`, and `premium.md` service tier definitions to `templates/tiers/`, unblocking the Client Onboarding workflow described in `agents/operations/client-onboarding.md`.
- **Context:** Both questions asked whether (and with what service definitions) the tier templates referenced by the Client Onboarding agent should be implemented. The shipped templates answer this implicitly with concrete positioning, feature, and limit definitions per tier. Verified on 2026-07-07: all three files exist with substantive, non-placeholder content.
- **Impact:** Both questions removed from `CLARIFICATIONS.md`. The `templates/tiers/` half of the "Infrastructure Asset Implementation Gap" limitation in `REQUIREMENTS.md` moved to the Remediated archive; the remaining Operating Profile half was renamed to "Operating Profile Starter Set Partially Implemented" (only `standard-operating-profile.md` of the three profiles approved by Decision [2026-06-19-0011] exists so far).
