...
### ❓ Question [2026-06-02] - Executive Assistant Service Is Undocumented in Requirements
**Context:** The requirements describe repository utilities, generators, and reference examples, but `REQUIREMENTS.md` never documents `tools/executive-assistant/` — a deployable Express/Cloud Run service that exposes a Gmail Pub/Sub push webhook (`POST /` in `server.js`) plus an admin REST API (`/api/queue`, `/api/resolution`, `/api/rules`, `/api/stats`, `/api/logs`) backed by `delta-sync.js`, `state-manager.js`, `triage-router.js`, and `oauth-manager.js`. The only existing reference to the tool (clarification [2026-05-02]) is narrowly about audit-log emission.
**Ambiguity / Drift:** A substantial deployable service with its own API contract and Fetch-on-Demand env-var dependencies (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, `REDIRECT_URI`, `PORT`) exists in code with no requirement, no documented contract, and no source-of-truth classification. The risk is an undocumented production surface and ambiguity over whether it is a "reference example" governed by §8 ("Reference Examples Must Tell the Truth") or a first-class deliverable.
**Question for Product Owner:** Should `tools/executive-assistant/` be formally documented in `REQUIREMENTS.md` as a reference example (subject to the §8 truthfulness rules) with its REST/Pub-Sub contract and required env vars captured, or should it be reclassified or removed?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Add a `REQUIREMENTS.md` subsection and a `tools/executive-assistant/README.md` documenting the Cloud Run service: the `POST /` Pub/Sub webhook, the `/api/*` admin endpoints, and the required Fetch-on-Demand environment variables, labeled per the §8 reference-example truthfulness rules.*

### ❓ Question [2026-06-02] - Branch-Source Enforcement Gate Allowlist Drift
**Context:** Requirements §7 (Governance) mandates the canonical `develop → main` flow and §9 mandates CI enforcement on `develop` and `main`. The workflow `.github/workflows/require-develop-source.yml` enforces that PRs to `main` originate from `develop`, but hardcodes a bypass allowlist of branch prefixes (`doc-workflow-audit-drift*`, `doc-requirements-refinement*`, `doc-requirements-drift-audit*`, `doc-reality-check*`, `doc-requirements-update*`).
**Ambiguity / Drift:** Neither this enforcement gate nor its `doc-*` bypass allowlist is documented in `REQUIREMENTS.md` or `DECISION_LOG.md`. The allowlist is brittle: it does not include the Doc agent's current branch namespace (e.g. `claude/serene-newton-*`), so legitimate Doc PRs to `main` would be rejected, while the bypass mechanism itself relaxes the mandated flow with no recorded decision. This is distinct from the pending "Branch Protection Audit" question, which concerns `scripts/audit-repo.js` checking GitHub-side protection.
**Question for Product Owner:** Should the Doc-agent bypass be formalized as a single documented prefix (e.g. `doc/*`), parameterized rather than hardcoded, and recorded in `REQUIREMENTS.md` §7 plus `DECISION_LOG.md`?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Update `.github/workflows/require-develop-source.yml` to use a single documented bypass prefix (e.g. `doc/*`), document the develop→main enforcement gate and its bypass policy in `REQUIREMENTS.md` §7, and record the decision in `DECISION_LOG.md`.*

### ❓ Question [2026-06-02] - Executive Assistant Delta-Sync Is a Non-Functional Stub
**Context:** On each Gmail Pub/Sub event, `tools/executive-assistant/server.js` calls `deltaSync.syncMailbox()` and logs `[Triage Router] Execution Complete`, implying that new-message history is processed and triaged.
**Ambiguity / Drift:** `delta-sync.js` `syncMailbox()` only calls `gmailClient.getProfile()` and returns `{ status: 'INITIALIZED', historyId }`. It never calls `getHistory()`, never invokes the injected `triageRouter.route()`, and never persists offsets via `stateManager`. The advertised delta-sync and triage behavior is unimplemented, and the success log is misleading — a vagueness/edge-case gap between the implied contract and the implemented behavior.
**Question for Product Owner:** Is the executive-assistant intended to be a functional reference implementation (so `syncMailbox` must fetch history and route messages) or an illustrative skeleton (so it should be labeled ILLUSTRATIVE per the §8 / `AGENTS.md` labeling rule and the misleading log corrected)?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Either implement `delta-sync.js` `syncMailbox()` to fetch Gmail history, route each new message through `triageRouter.route()`, and persist the offset via `stateManager`; or mark the executive-assistant as ILLUSTRATIVE and correct the misleading "[Triage Router] Execution Complete" log line.*
