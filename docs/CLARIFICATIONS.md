...
### ❓ Question [2026-07-10] - Merge-Gate Wildcard Re-Accretion and Overdue develop → main Release
**Context:** The requirements state in §7 that "`develop` is the ONLY valid PR source into `main`" and that any hotfix exception must be "a single explicit entry... never a broad wildcard" (Decisions [2026-07-03-0001], [2026-07-07-0002]). However, `main`'s copy of `.github/workflows/require-develop-source.yml` has re-accreted two wildcard exceptions (`doc-requirements-uplift-*`, `doc-requirements-refinement-*`) via direct-to-main automation commits `a0a7fad` and `e54275a`, and as of 2026-07-10 `develop` and `main` have diverged (8 commits only on `develop`, 7 only on `main`).
**Ambiguity / Drift:** The doc-automation bot keeps opening PRs straight to `main`, re-raising questions already resolved on `develop` by the [2026-07-07-000x] decision series. The two branches now carry conflicting `CLARIFICATIONS.md`/`DECISION_LOG.md`/`REQUIREMENTS.md` states, so the next release PR will hit merge conflicts in every governance document, and the weekly `develop → main` release cadence mandated by §7 has not run since the invariant was codified.
**Question for Product Owner:** Should the two wildcard exceptions be removed from `main`'s merge gate and the doc-automation bot reconfigured to base and target `develop`, and who owns the one-time reconciling `develop → main` release PR (a human, or a scheduled routine as §7 suggests)?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Reconcile the diverged governance docs in a `develop → main` release PR, remove the `doc-requirements-uplift-*` and `doc-requirements-refinement-*` wildcard exceptions from `.github/workflows/require-develop-source.yml`, and retarget the doc-automation runs to base and target `develop`.*

### ❓ Question [2026-07-10] - Compassion Lens Governance and the Undefined Anti-Replacement Rule
**Context:** The requirements mandate that framework assets follow canonical Markdown contracts, but `value-lenses/compassion-lens.json` (commit `bb381d2`) ships as a JSON-only value lens whose description says it enforces "the Project NoéMI Anti-Replacement rule across the entire fleet." The generator injects only `.md` lenses, so this lens never reaches `GEMINI.md`/`CLAUDE.md`, and the "Anti-Replacement rule" is defined nowhere in `REQUIREMENTS.md`, `AGENTS.md`, or `docs/frameworks/`. Meanwhile three personas (`roi-auditor`, `student-success-coach`, `qa-risk-manager`) already carry constraints derived from it.
**Ambiguity / Drift:** The fleet has persona-level safety constraints (Workforce Uplift Constraint, Contextual Forgiveness Rule, Precarity Consciousness) whose governing rule and value lens are invisible to the audit, generation, and requirements layers. The `american-dream` lens demonstrates the canonical pattern: a `.md` lens contract with a `.json` machine-readable companion.
**Question for Product Owner:** Should the compassion lens be promoted to a canonical Markdown lens (`value-lenses/compassion-lens.md` per `LENS_TEMPLATE.md`, keeping the JSON as a companion), and should the "Anti-Replacement rule" it references be formally defined in `REQUIREMENTS.md` or the Value Lens framework docs?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Author `value-lenses/compassion-lens.md` from the JSON definition following `LENS_TEMPLATE.md`, keep `compassion-lens.json` as its machine-readable companion, define the Anti-Replacement rule in the Value Lens framework documentation, and regenerate the context files and golden fixtures.*
