...
### ❓ Question [2026-05-28] - CLARIFICATIONS.md Backlog Normalization

**Context:** `docs/CLARIFICATIONS.md` has grown to over 587 lines containing 40+ open questions. The file header (written April 2026) still reads "There are no open product clarifications blocking the repository at this time," which is no longer accurate. Multiple autonomous implementation cycles since April have resolved items in code (Node.js 24 Docker images, SecretOps auth verification, API path standardization, skills audit coverage) without corresponding PO answers, so those questions remain as `[LEAVE BLANK FOR HUMAN TO FILL]` even though the work is done. Additionally, at least 10 near-identical questions appear 2–3 times under different dates.
**Ambiguity / Drift:** The current state of `CLARIFICATIONS.md` makes it difficult for the Product Owner to identify which questions genuinely require a decision vs. which were overtaken by implementation. This degrades the quality of the feedback loop between the autonomous doc agent and the product team.
**Question for Product Owner:** Should Doc perform a wave-2 normalization pass to: (1) close implementation-resolved questions by moving them to `DECISION_LOG.md` as "Superseded by autonomous implementation," (2) deduplicate near-identical questions retaining only the most recent version, and (3) update the file header to accurately reflect the current open question count?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Perform a full normalization of `docs/CLARIFICATIONS.md`: archive implementation-resolved items to `docs/DECISION_LOG.md`, deduplicate near-identical questions, and update the file header to reflect the live backlog size.*

### ❓ Question [2026-05-28] - Branch Allowlist Hardcoding in require-develop-source.yml

**Context:** `.github/workflows/require-develop-source.yml` enforces the `develop → main` governance rule by checking that the source branch of any PR to `main` is either `develop` or matches a hardcoded list of patterns: `doc-workflow-audit-drift*`, `doc-requirements-refinement*`, `doc-requirements-drift-audit*`, `doc-reality-check*`, `doc-requirements-update*`. These patterns reflect specific AI agent doc branch naming conventions that bypass the develop gate.
**Ambiguity / Drift:** As new agent branch naming conventions emerge (e.g., `claude/jolly-curie-wgGo6` from the Claude Code agent, community PRs, or security hotfixes), each new pattern requires a manual workflow file edit. This creates ongoing governance maintenance overhead and risks either blocking legitimate agent PRs or silently widening the bypass surface over time — both outcomes contradict Decision [2026-05-20] Branch Protection: Mandatory Enforcement.
**Question for Product Owner:** Should the workflow be refactored to use a configurable source (e.g., a GitHub repository variable `NOEMI_ALLOWED_SOURCE_PATTERNS` or a `.noemi-branch-policy.json` checked into the repo root) rather than a hardcoded string match, so the allowlist can be updated without touching CI YAML?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Refactor `.github/workflows/require-develop-source.yml` to read its source-branch exception patterns from a GitHub repository variable or a `.noemi-branch-policy.json` configuration file, eliminating hardcoded pattern maintenance.*

### ❓ Question [2026-05-28] - New Agent Documentation Mirror Gap

**Context:** Three agents were added to the repository in the 2026-05-25 wave via community contribution and merged to `main`: `agents/coding/architect/core.md`, `agents/coding/bolt/go.md`, and `agents/education/student-success-coach.md`. The `docs/agents/coding/` directory only contains guide content for `bolt/` and `sentinel/`. There is no `docs/agents/coding/architect/` directory, no `docs/agents/coding/bolt/go.md` file, and no `docs/agents/education/` domain directory at all. Per Decision [2026-04-02], mirroring is interpreted "at the directory and guide level, not as a mandatory 1:1 file symlink mirror," but the gap now spans two entire domains (education) and one new specialist variant (Architect).
**Ambiguity / Drift:** Without documentation mirror content, builders cannot find operational context for these agents through the `docs/agents/` discovery path. Additionally, `scripts/audit-repo.js` has no check to surface missing mirrors, so new agents can be added indefinitely without triggering a governance warning.
**Question for Product Owner:** Should Doc be tasked with (1) creating lightweight stub guide entries in `docs/agents/coding/architect/`, `docs/agents/education/`, and extending `docs/agents/coding/bolt/` for the Go variant, and (2) adding an optional `--warn-missing-mirrors` flag to `scripts/audit-repo.js` to surface agents without documentation coverage?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Create stub documentation guides in `docs/agents/coding/architect/`, `docs/agents/education/student-success-coach/`, and `docs/agents/coding/bolt/` (Go variant), then add an optional audit flag to `scripts/audit-repo.js` to detect and surface agents missing a documentation mirror.*
