...
- **Docker e2e Skip Behavior**: The Docker e2e suite `tests/e2e/docker-smoke.test.js` skips runtime checks if Docker is absent rather than failing, potentially masking configuration errors in CI environments.
- **Logging Protocol Gap**: `logging-mcp` is a dual-backend draft (Loki/n8n) not yet active in `mcp.config.json`. Reference services like `dashboard-ingest.js` lack alignment with its schema.
- **Memory-Code Synchronization Drift**: Multiple remediations reported as completed (shared `audit_logger.js`, parameterized `sync-upstream.sh`, enhanced Agent Index extraction) remain missing or incomplete in the live filesystem.
...
- **Smoke Test Variable Validation Gap**: `tests/examples-smoke.test.js` lacks validation logic for `NOEMI_DOCKER_SMOKE_*` environment variables mandated by Requirement 9.
- **AI Model Baseline Drift**: `examples/docker/agent.py` is pinned to `gemini-2.0-flash`, violating the `gemini-2.5-flash` canonical baseline mandate.

### 2. Structural & Architectural Drift
- **Audit Script Enforcement Depth**: `scripts/audit-repo.js` lacks structural audits for `mcp-protocols/`, `value-lenses/`, and `operating-profiles/`. It also fails to verify branch protection status and referential integrity for internal links and skill paths.
...
- **Substantive Content Boilerplate**: 100% of reusable skills in `skills/` contain "TBD" placeholders for mandatory `Data Inventory` and `Refusal Criteria` sections. Agent personas often use identical boilerplate, satisfying structural audits but failing the Refusal Principle mandate for role-specific precision.
- **Audit Script Structural Limitation**: `scripts/audit-repo.js` fails to detect "TBD" or hollow placeholders, allowing safety-deficient assets to pass the audit.
- **Audit Script JSON Schema Blindness**: `scripts/audit-repo.js` verifies valid JSON in `Audit Log` sections but fails to validate mandated fields (`task`, `inputs`, `actions`, `risks`, `result`).
- **Internal Tool Observability Gap**: Node.js tools in `tools/` (e.g., `executive-assistant`) lack structured JSON Audit Log emission to `stderr`, missing the shared `audit_logger.js` utility.
- **Operating Profile Substantive Gap**: `operating-profiles/` contains only templates and no baseline profiles, resulting in empty injection blocks in generated context files.
...
- **Internal Referential Integrity Gap**: The repository lacks automated verification that internal markdown links or `**Skill:**` path references point to valid, existing files.
