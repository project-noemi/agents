...
- **Test Suite Gaps** (Remediated: 2026-05-29): `tests/examples-smoke.test.js` now asserts the presence of all `NOEMI_DOCKER_SMOKE_*` variables in `.env.template` (Decision [2026-05-29-0001]).
...
- **Sync Script Hardcoding** (Remediated: 2026-05-29): `scripts/sync-upstream.sh` now reads `NOEMI_UPSTREAM_URL` and `NOEMI_ORG_NAME` from the environment with sensible defaults (Decision [2026-05-29-0002]). Forks no longer need to patch the script.
...
- **Docker Smoke Test Variable Validation Gap** (Remediated: 2026-05-29): Closed by Decision [2026-05-29-0001] — see remediated entry above.
- **Sync Script Parameterization Gap** (Remediated: 2026-05-29): Closed by Decision [2026-05-29-0002] — see remediated entry above.
