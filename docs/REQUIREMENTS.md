...
- **Testing Toolchain Drift**: The sub-tool `tools/executive-assistant/` utilizes `vitest` as its testing framework (as defined in its `package.json`), representing a toolchain drift from the root-level baseline that mandates the native Node.js test runner for standard verification.
...
- **Heading Inconsistency**: The mandatory Diligence heading is "Rules & Constraints" in agents but "Rules & Constraints (4D Diligence)" in skills. Approximately 90% of active agent personas (e.g., `agents/operations/linux.md`, `agents/education/student-success-coach.md`) implement `## Rules & Constraints (4D Diligence)`, which deviates from the core heading specification in `AGENTS.md` and complicates automated audit/generation parsing logic.
