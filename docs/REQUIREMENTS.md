...
- The public documentation must include a reusable **Phase 0 Assessment Kit** with exactly 9 assessment files (including `README.md`, templates, and assessment guides):
...
- **Internal Tool Observability Gap**: While the shared `scripts/audit_logger.js` utility is available, Node.js tools like `executive-assistant` (`tools/executive-assistant/server.js`, lines 19, 44, 49, 103, 107, 121, 125, 150, 156, 161) and reference services like `dashboard-ingest.js` still rely on unstructured logging (`console.log`, `console.error`) and unauthenticated endpoints providing direct access to internal agent state. They have not yet adopted the mandated JSON Audit Log emission to `stderr` or authorization checks for operational events. This includes events like `[Pub/Sub] Notification received` and `[Triage Router] Execution Complete`.
...
- **Phase 0 Audit Gap**: `scripts/audit-repo.js` lacks logic to verify the presence of the exactly 9 mandated assessment files in `docs/phase-zero-assessment/` (such as `network-security-assessment.md`, `readiness-rubric.md`, `PRACTITIONER_NOTES.md`), violating Requirement §1.
...
- **Substantive Content Baseline (The "TBD" Problem)**: 100% of active reusable skills in `skills/` (e.g., `skills/reporting/alert-notify.md`) and several framework assets under `value-lenses/` (e.g., `value-lenses/care-continuity.md`, `value-lenses/balanced-enterprise.md`, and `value-lenses/performance-efficiency.md` which have 'TBD' owner fields) use "TBD" placeholders or boilerplate for core sections, bypassing the D2 (Description) and Refusal Principle mandates.
