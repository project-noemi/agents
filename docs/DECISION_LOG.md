...
## [2026-07-02] Bulk Clarification Resolution — 2026-04 through 2026-06 Backlog

- **Decision:** The following clarifications from the April–June 2026 backlog are resolved with the reasoning captured below. Follow the industry-practice → best-practice → recommendation pattern per the "propose-then-review" preference. Implementation lands in the same PR that authors this entry so `REQUIREMENTS.md` and the codebase stay in lockstep.
- **Context:** The clarification queue accumulated ~45 open questions across three principal themes: (a) observability/audit-logger gaps, (b) audit-script rigor (schema, TBD placeholders, referential integrity, Phase 0), and (c) missing scaffolding (red-team vectors, tier templates, operating profiles). Each theme had multiple restated variants; addressing them one-by-one would produce dozens of tiny PRs with no closure. This bulk decision closes the theme and lands the enabling implementation.
- **Answers (question → decision → rationale):**
  - **[2026-04-05] Red Team Gauntlet Test Vector Absence** and **[2026-06-10] / [2026-06-15] serialization strategy** → Serialize the 5 prose test cases into `examples/red-team-gauntlet/test-vectors.json`. Rationale: single-file JSON is the simplest machine-readable shape; automated Client Onboarding validation can `require()` it directly; category-split files can be added later without breaking consumers.
  - **[2026-06-10] / [2026-06-15] Empty Tier Templates** → Author `templates/tiers/basic.md`, `standard.md`, and `premium.md` with the tier definitions embedded in this PR. Rationale: templates were referenced by the Client Onboarding persona but non-existent; providing the reference tier ladder unblocks the Explorer path without committing to bespoke tier logic.
  - **[2026-06-15] Operating Profile Baseline Absence** → Ship `operating-profiles/standard-operating-profile.md` as the neutral baseline. Rationale: the mandate asks for framework injection; the framework needs at least one operational profile to produce non-empty injection blocks.
  - **[2026-06-10] / [2026-06-11] / [2026-06-18] / [2026-06-19] `scripts/audit_logger.js` Absence** → Implement the shared utility in this PR with both CommonJS (`scripts/audit_logger.js`) and ESM shim (`scripts/audit_logger.mjs`) entry points, exposing the canonical 5-field Audit Log schema (`task`, `inputs`, `actions`, `risks`, `result`). Rationale: the schema is fully specified in `AGENTS.md`; a shared utility beats N ad-hoc emitters; dual-module support closes the ESM/CJS mismatch called out in [2026-06-19] Resilience Helper Module System Mismatch.
  - **[2026-06-19] Resilience Helper Module System Mismatch** → Add `scripts/resilience_helpers.mjs` as an ESM shim over the existing CommonJS implementation. Rationale: preserves the CJS API for existing consumers while unblocking ESM-native tools like `tools/executive-assistant/` without a rewrite.
  - **[2026-06-11] / [2026-06-18] `verify-env.sh` / `.ps1` Fatal Error in Docker Mode** → Update both scripts to `exit 1` when SecretOps CLI is missing or unauthenticated **only in `docker` mode**; other modes continue to warn. Rationale: `AGENTS.md` mandates this behavior explicitly; other modes still support local exploration.
  - **[2026-05-29] / [2026-06-11] `scripts/sync-upstream.sh` Hardcoded Identity** → Refactor to source `NOEMI_UPSTREAM_URL`, `NOEMI_UPSTREAM_REMOTE`, `NOEMI_SYNC_BRANCH`, and `NOEMI_ORG_NAME` from the environment with upstream-safe defaults. Rationale: matches AGENTS.md "Script Parameterization" mandate; zero-friction for forks; no CI behavior change on the reference repo.
  - **[2026-05-29] / [2026-06-11] Docker Smoke Test Variable Validation** → Add a test case to `tests/examples-smoke.test.js` asserting `NOEMI_DOCKER_SMOKE_*` variables are inventoried in `.env.template`. Rationale: closes Requirement §9 blind spot with a fast structural check; deeper e2e validation continues to live in `tests/e2e/`.
  - **[2026-06-19] SecretOps Provider Bias in Smoke Tests** → Update the vault-reference assertion to match either `op://` OR `infisical://`. Rationale: `AGENTS.md` mandates dual-provider support; hard-coding one provider creates false failures.
  - **[2026-06-18] E2E `FORCE_DOCKER_SMOKE` Mandatory Mode** → Add `FORCE_DOCKER_SMOKE=true` environment flag to `tests/e2e/docker-smoke.test.js` that makes missing Docker fatal (exit 1). Default remains clean skip. Rationale: preserves local dev ergonomics while giving CI a way to catch "false green" configurations.
  - **[2026-06-18] / [2026-06-22] Audit Script JSON Schema Validation** → Add mandatory 5-field validation (`task`, `inputs`, `actions`, `risks`, `result`) in `scripts/audit-repo.js` for every Audit Log block. Rationale: the schema is already the canonical shape; validating presence closes the D4 Diligence layer at the gate.
  - **[2026-06-01] / [2026-06-15] / [2026-06-22] Audit Script Placeholder Rejection** → Implement placeholder detection ("TBD"/"placeholder") in `Data Inventory` and `Refusal Criteria` sections. Emit as **warnings by default**; make fatal when `NOEMI_AUDIT_SUBSTANTIVE=strict` is set. Rationale: 100% of skills currently contain "TBD" — flipping to fatal immediately would break CI without a substantive remediation PR; warn-then-enforce ships the enforcement infrastructure now and lets the switch flip once the skill library is remediated. Progressive tightening matches "small chunks, close loops."
  - **[2026-05-02] / [2026-05-15] / [2026-06-17] Skill-to-Agent & MCP Config Referential Integrity** → Extend `scripts/audit-repo.js` to verify every `active_mcps` and `active_skills` entry in `mcp.config.json` resolves to an existing file. Full internal-link referential integrity across all markdown remains a future scope (larger surface). Rationale: config-scoped check catches the highest-impact drift with a bounded implementation.
  - **[2026-05-02] / [2026-05-19] Internal Tool Observability Standard** → All Node.js internal tools and reference services must use `scripts/audit_logger.js` (CJS) or `scripts/audit_logger.mjs` (ESM) to emit structured JSON Audit Logs to stderr. `scripts/audit-repo.js` and `examples/gatekeeper-deployment/dashboard-ingest.js` refactored in this PR. `tools/executive-assistant/server.js` refactor deferred (larger touch surface; ESM shim now available). Rationale: normalize the emission channel; ship the highest-visibility services now.
  - **[2026-05-02] Agent Index Role Truncation** → Update `scripts/context_helpers.js` to extract the full first paragraph of `## Role` (not the first sentence). Preserves technical names with dots (e.g., "Next.js 16"). Rationale: matches AGENTS.md "Agent Index Richness" mandate; unblocks descriptive accuracy for downstream models.
  - **[2026-06-20] Phase 0 Assessment Kit Audit Coverage** → `scripts/audit-repo.js` now verifies all 8 mandated files exist under `docs/phase-zero-assessment/`. Rationale: Phase 0 is the buyer's front door per REQUIREMENTS.md §1; missing files must fail the audit.
  - **[2026-06-19] Legacy Example Model Drift** → Update `examples/docker/agent.py` from `gemini-2.0-flash` to `gemini-2.5-flash`. Broader "audit for non-baseline model pins" (Q [2026-05-20]) is deferred until a repo-wide grep confirms the full pin inventory. Rationale: closes the concrete drift now; the generic audit is a separate incremental step.
  - **[2026-05-17] Generator Script Redundancy** → Retain `scripts/generate_all.js` as the canonical entry point. Legacy `generate_gemini.js` and `generate_claude.js` are kept but should be considered deprecated shims (already delegate to shared helpers). No file removal in this PR to avoid breaking downstream call-sites that may reference them; a follow-up PR can remove after a deprecation notice cycle. Rationale: risk-aware — a bulk delete could break external consumers documented elsewhere.
- **Deferred (requires PO / stakeholder decision):**
  - **[2026-04-03] ROI Auditor Baseline Data Access** — mechanism choice (MCP read vs local JSON) is a product-shape decision, not a code cleanup.
  - **[2026-04-05] Fleet Dashboard Retention Policy Drift** — 90d vs 90d+1yr with downsampling is a cost/policy tradeoff.
  - **[2026-04-05] Fleet Dashboard Multi-tenancy Implementation Gap** and **[2026-04-23] Gatekeeper Mutating Actions** — expanding the reference implementation to include registry, async verification, and mutating actions is a multi-week architectural build that needs product sequencing.
  - **[2026-04-26] / [2026-05-10] / [2026-05-29] / [2026-06-17] Substantive Persona/Skill Remediation** — bulk rewrite of 100% of skills' `Data Inventory` and `Refusal Criteria` is intentionally left to domain owners; audit warnings now surface the remediation queue. Once complete, flip `NOEMI_AUDIT_SUBSTANTIVE=strict` in CI.
  - **[2026-05-02] / [2026-05-22] Casdoor Identity Integration Logic** — implementation depends on the multi-tenant fleet direction (see deferred items above).
  - **[2026-05-01] / [2026-05-20] Automated Node/Model Baseline Sweeps** — repo-wide grep-based CI checks are useful but scope-sensitive; deferred until we have a stable list of "allowed exceptions."
  - **[2026-05-02] Naming Convention Regex Audit** — needs a scope decision (which directories, which patterns) before implementation.
  - **[2026-05-13] RFP Split Naming Convention Remediation** — bulk file rename affects any external doc that links to those PDFs; needs owner sign-off.
  - **[2026-05-01] Persona Journal Section Standardization** — adds a mandatory heading; would break 100% of personas until remediated. Product-shape decision.
  - **[2026-05-15] Test Suite Reinforcement of API Path Drift** — the api-path check is now covered by the existing `dashboard-ingest` smoke test; no further work required, but leaving marked "deferred" pending verification sweep across all consumers.
  - **[2026-05-28] / [2026-06-01] / [2026-06-18] Substantive Skill Remediation** — same reason as persona/skill remediation above; deferred to domain owners.
  - **[2026-06-01] Memory-Code Synchronization Drift** — the "2026-05-31 remediations" question about missing files is resolved by this PR (the enumerated files now exist).
  - **[2026-06-17] MCP Protocol Contract Audit & Value-Lens/Operating-Profile Structural Audit** — needs a mandatory-heading contract for MCP protocol specs before enforcement can land.
  - **[2026-06-20] Rules & Constraints Heading Inconsistency** — cosmetic mismatch (`Rules & Constraints` vs `Rules & Constraints (4D Diligence)`); standardizing requires updating either 26 agents or 8 skills. Left for a targeted follow-up PR.
  - **[2026-06-20] / [2026-06-22] Learning Agent Feedback Loop & Admin Control Plane Formalization** — governance of a feature drift in `tools/executive-assistant/`. Product owner must decide whether to formalize or remove; not a code cleanup.
  - **[2026-06-19] Resilience Helper Persona Integration** — bulk-update all network-bound personas to reference `withRetry`; scope-sensitive.
  - **[2026-06-27] Generator Fail-Fast Policy** — needs a policy call on which framework-asset directories are mandatory (currently, missing `value-lenses/` yields an HTML comment; making that fatal is fine but is a policy shift, not a bug fix).
- **Impact:**
  - `docs/CLARIFICATIONS.md` shrinks to only the deferred product-owner questions above.
  - `docs/REQUIREMENTS.md` §Current Known Limitations updates: `audit_logger.js` (remediated), Agent Index Descriptive Truncation (remediated), SecretOps Docker Fatal (remediated), Sync Script Identity Drift (remediated), Provider Bias in Smoke Tests (remediated), Missing Red Team Vectors (remediated), Missing Tier Templates (remediated), Missing Operating Profile Baseline (remediated), Node/Model Legacy Pin Drift (single-file remediation), Audit Log Schema Blindness (remediated), Audit Script File Verification Gap (remediated), Phase 0 Audit Gap (remediated), Internal Tool Observability Gap (partially remediated), Resilience Helper Module Mismatch (remediated via ESM shim).
