...
## [2026-06-29-0001] Audit Script Coverage and Agent Index Implementation Wave

- **Decision:** Implement the implementable portions of Decision [2026-06-19-0012] in `scripts/`:
  - `scripts/context_helpers.js` now extracts the **full first paragraph** of the `## Role` section for the Agent Index (newline-collapsed, capped at 400 chars). Closes the long-standing "Agent Index Accuracy Drift" / "Agent Index First-Paragraph Extraction Pending" entries.
  - `scripts/audit-repo.js` gains four supplementary checks (warning-only, do not fail the audit yet so they can land without forcing a fleet-wide cleanup): (1) Node 24 baseline scan across `Dockerfile` and `docker-compose.yml`; (2) AI model baseline scan across `examples/` and `tests/` enforcing `gemini-2.5-flash`; (3) slug-based filename audit across `docs/`, `agents/`, `skills/`, `examples/`, `tools/` (with carve-outs for `Dockerfile*`, dotfiles, PascalCase React components, generated coverage/dist trees); (4) `mcp.config.json` referential integrity for `active_mcps` and `active_skills`.
  - `examples/docker/agent.py` is bumped from `gemini-2.0-flash` to `gemini-2.5-flash` (canonical baseline). Resolves CLARIFICATIONS Q [2026-06-19] AI Model Drift inline so the new audit check passes on a clean tree.
- **Context:** The umbrella scope was approved in Decision [2026-06-19-0012]; this entry records the actual landing. Refusal Criteria substantive enforcement and skill-to-agent reference integrity remain deferred — Refusal-Criteria enforcement requires a domain-aware substantive check (premature regex would create false positives), and skill-to-agent reference integrity requires parsing each persona's Workflow section for `**Skill:** [path]` patterns which is non-trivial relative to its current value.
- **Impact:** Closes "Agent Index Accuracy Drift", the explicit "Agent Index First-Paragraph Extraction Pending" entry, and the "AI Model Baseline Drift" entry. Downgrades "Audit Script Coverage Expansion Pending" from full-stop to partially-shipped. The supplementary audits will be promoted from warning to fatal in a follow-up PR once the residual warnings are cleaned up.
