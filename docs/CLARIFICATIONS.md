...
- [2026-04-27] Resolved the April-May 2026 architectural-alignment backlog (24 questions). Durable answers normalized into `DECISION_LOG.md` under entries dated `2026-04-27`. Topics covered: ROI Auditor baseline data access, logging-mcp / Audit Log layering, onboarding directories, Fleet Dashboard API path, Value Lenses / Operating Profiles in context generation, InfluxDB backend support, SecretOps reference file split, smoke-test coverage expansion, Fleet Dashboard retention policy, Red Team Gauntlet starter vectors, reference-service Audit Log compliance, Fleet Dashboard multi-tenancy phasing, audit-repo.js JSON schema validation, skill contract audit enforcement, Data Inventory in skill contract, substantive persona content remediation strategy, reference example asset population, NOEMI_DOCKER_SMOKE_* smoke checks, Gatekeeper dry-run mutating actions, Node.js 24 Docker baseline, and persona Journal mandate.
