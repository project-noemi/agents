...
### 11. Licensing and Distribution Model Must Stay Coherent
...
- The repository publicly identifies as **Fair Source** (fair.io), declared via the SPDX `license` field in `package.json` (`FSL-1.1-Apache-2.0`).
- The **NoéMI Transparent™** mark and its five-clause guarantee are defined in [`TRANSPARENT_SOURCE.md`](TRANSPARENT_SOURCE.md); legal rights flow from the FSL, trust norms flow from the mark. The guarantee is scoped to spec-and-governance transparency (deterministic context generation, golden fixtures, persona Audit Log contracts, public decision log) — not model-weight or training-data disclosure.
- **Tagged GitHub releases are the canonical published versions**: the FSL two-year conversion timeline is measured per tagged release, so a release must be tagged at (or after) every licensing-relevant milestone. Versions published before the relicense remain available under SSPL v1.
- Self-referential documentation must describe the project as "Fair Source", not "open source"; references to third-party open-source components (Infisical, Casdoor, self-hosted models) are unaffected.
...
- **Operating Profile Starter Set Incomplete**: of the three approved starter profiles (Decision [2026-06-19-0011]), only `operating-profiles/standard-operating-profile.md` has shipped; `high-trust-regulated.md` and `fast-iteration-startup.md` remain unimplemented, and the JSON sovereign profile remains invisible to the generator (see Sovereign JSON Asset Layer Governance Gap).
- **FSL Conversion Clock Anchor Missing**: Decision [2026-07-05-0004] requires a tagged release at the relicense merge commit so the first FSL two-year conversion timeline has an unambiguous start date. The only existing tag (`v0.1.0`) points at the pre-relicense PR #162 merge; no post-relicense release has been tagged, leaving the conversion clock without a legal anchor (see Clarification [2026-07-06] FSL Conversion Clock Anchor).
- **Undocumented Top-Level Asset Layers**: the `infrastructure/` (Phase 0 secret-ops boilerplate, LEGACY-labeled Python) and `n8n-templates/` (Layer B Dynamic Labs importable workflows) directories exist outside the documented repository layout, the context-generation pipeline, and the audit gate (`scripts/audit-repo.js` scans only `agents/` and `skills/`). Their assets carry correct model pins (`models/gemini-2.5-flash`) and LEGACY labels, but no requirement governs their structure or placement (see Clarification [2026-07-06] Top-Level Asset Layer Governance).
...
- **Empty Service Tier Templates** (Remediated: 2026-07-05, verified 2026-07-06): `templates/tiers/` now ships substantive `basic.md`, `standard.md`, and `premium.md` templates (salvaged from stale PR #254 via PR #264) defining per-tier MCP provisioning, skills, onboarding checks, and graduation criteria. The templates deliberately contain no pricing; commercial pricing remains outside the public reference architecture (Decision [2026-07-06-0001]).
