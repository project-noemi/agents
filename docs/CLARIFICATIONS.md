...
### ❓ Question [2026-06-12] - Executive Assistant Test Framework Divergence
**Context:** `REQUIREMENTS.md` §9 and the Runtime and Tooling Requirements state that "the built-in Node test runner is the primary validation framework for repository contracts and smoke tests," and the root `package.json` wires every gate through `node --test` (`npm test`, `test:contracts`, `test:e2e`).
**Ambiguity / Drift:** `tools/executive-assistant/package.json` instead declares `"test": "vitest run"` with `vitest`, `@vitest/coverage-v8`, and `supertest` as devDependencies. The root `npm test` only globs `tests/*.test.js`, so the executive-assistant suite (`tools/executive-assistant/tests/*.test.js`) never runs under `npm run validate` — it is orphaned from the canonical validation contract and uses a non-baseline framework.
**Question for Product Owner:** Should the executive-assistant tool be migrated to the built-in Node test runner to match the repository baseline, or should `REQUIREMENTS.md` formally carve out an exception permitting sub-package tooling (vitest) for runnable application tools in `tools/`? Either way, how should these tests be reachable from the top-level validation gate?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Either port `tools/executive-assistant/tests/` to `node --test` and add a `test:tools` script to the root `package.json` that `npm run validate` invokes, or document an explicit vitest exception for runnable `tools/` sub-packages in `REQUIREMENTS.md` and wire their suites into a CI step.*

### ❓ Question [2026-06-12] - Undocumented Executive Assistant Reference Service
**Context:** `tools/executive-assistant/` is a fully implemented reference service: an Express app (`server.js`) that receives Gmail Pub/Sub push notifications on Cloud Run, runs a `TriageRouter` (VIP / IGNORE / STANDARD classification), an `OAuthManager` for Gmail token abstraction, a `DeltaSync` mailbox processor, and an admin REST API (`/api/queue`, `/api/rules`, `/api/resolution`, `/api/stats`, `/api/logs`) backing a Vite dashboard.
**Ambiguity / Drift:** `REQUIREMENTS.md` references this tool only once — as an "Internal Tool Observability Gap." Its architectural role, runtime model (Cloud Run + Pub/Sub), confidence-based "flaggedForReview" triage model, and HTTP surface are entirely undocumented, contradicting the repository's reference-architecture mandate and §8 "Reference Examples Must Tell the Truth."
**Question for Product Owner:** Should the executive-assistant be promoted to a documented reference example with its own requirements subsection (runtime, endpoints, triage/confidence model, secret dependencies), or is it intended to remain an undocumented internal scratch tool? If documented, which behaviors are contractual versus illustrative mock data (e.g., the hardcoded `firestoreThreadDocs` and `systemStats`)?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Add a `tools/executive-assistant/README.md` and a `REQUIREMENTS.md` subsection describing the service's Cloud Run/Pub/Sub runtime, triage routing model, admin API surface, and required environment variables, clearly labeling mock data structures as illustrative.*

### ❓ Question [2026-06-12] - Unauthenticated Mutating Admin Endpoints
**Context:** §7 (Governance and Trust Controls) makes Guardian-layer trust patterns first-class, the Gatekeeper reference path uses HMAC-signed submissions (Decision [2026-05-20]), and Casdoor is identified as the reference identity layer. In contrast, `tools/executive-assistant/server.js` exposes `PUT /api/rules` (mutates the live `triageRouter` VIP/ignore arrays) and `POST /api/resolution` (mutates thread state and execution logs) with no authentication, authorization, HMAC verification, or identity context.
**Ambiguity / Drift:** A reference service that performs state mutation over unauthenticated HTTP contradicts the project's stated trust posture and could be mistaken for an approved pattern by forks building on it.
**Question for Product Owner:** Should the executive-assistant's mutating admin endpoints require an auth layer (e.g., bearer/HMAC or the Casdoor JWT middleware discussed in the 2026-05-22 clarification) to align with §7, or should the service be explicitly labeled as an unauthenticated local-only demo not intended for the governed trust path?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Add authentication middleware (bearer token or HMAC, consistent with the Gatekeeper/Casdoor pattern) to the mutating `PUT /api/rules` and `POST /api/resolution` endpoints in `tools/executive-assistant/server.js`, or add a prominent "unauthenticated local demo" banner to the tool and its docs.*
