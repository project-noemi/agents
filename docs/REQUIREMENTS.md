...
- **Resilience Helper Integration Gap**: `scripts/resilience_helpers.js` exists as a canonical reference but is not consumed by any agent persona or reference service. Per Decision [2026-04-04] the local-filesystem build utilities (`audit-repo.js`, `generate_all.js`) are intentionally exempt (deterministic local reads do not benefit from backoff). The unaddressed gap is the network-I/O reference service `examples/gatekeeper-deployment/dashboard-ingest.js`, whose InfluxDB write uses a bare `fetch` with no exponential backoff and surfaces transient failures as an immediate HTTP 502 (see Clarification [2026-06-03] — Reference Ingest Service Lacks Backoff).
