...
- **Infrastructure Asset Implementation Gap**: `templates/tiers/` is now populated with `basic.md`, `standard.md`, and `premium.md` per Decision [2026-07-08-0001] (shipped in commit `9c19548`); the `Client Onboarding` agent's tier-template references resolve to real files. `operating-profiles/` contains one baseline Markdown profile (`standard-operating-profile.md`) plus the JSON sovereign profile invisible to the generator; the two remaining Markdown profiles from Decision [2026-06-19-0011] (`high-trust-regulated.md`, `fast-iteration-startup.md`) are still unimplemented, so the Operating Profile injection block is partial rather than empty.
