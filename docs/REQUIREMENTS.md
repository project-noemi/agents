...
- **Executive Assistant Service Undocumented**: `tools/executive-assistant/` implements a deployable Express/Cloud Run Gmail-triage service (a Pub/Sub `POST /` webhook plus `/api/queue`, `/api/resolution`, `/api/rules`, `/api/stats`, and `/api/logs` admin endpoints) with its own env-var contract (`GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, `REDIRECT_URI`, `PORT`), but is not documented as a requirement or reference example and has no source-of-truth classification (open clarification [2026-06-02]).
- **Branch-Source Gate Allowlist Drift**: `.github/workflows/require-develop-source.yml` enforces the canonical `develop → main` flow required by §7, but relies on a hardcoded `doc-*` branch bypass allowlist that is undocumented in this file and `DECISION_LOG.md` and does not match the Doc agent's current branch namespace (open clarification [2026-06-02]).
- **Executive Assistant Delta-Sync Stub**: `tools/executive-assistant/delta-sync.js` `syncMailbox()` returns `{ status: 'INITIALIZED' }` only — it does not fetch Gmail history, route messages through `triage-router.js`, or persist state via `state-manager.js`, despite `server.js` logging `[Triage Router] Execution Complete` on every Pub/Sub event (open clarification [2026-06-02]).
