...
- **Merge-Gate Wildcard Re-Accretion and develop/main Divergence** (Answered 2026-07-10-0002; implementation pending on the reconciling release PR): Requirement §7 mandates that `develop` is the only valid PR source into `main` and that any hotfix exception must be "a single explicit entry... never a broad wildcard," but `main`'s copy of `.github/workflows/require-develop-source.yml` has re-accreted two wildcard exceptions (`doc-requirements-uplift-*` and `doc-requirements-refinement-*`) via direct-to-main automation commits `a0a7fad` (2026-07-07) and `e54275a` (2026-07-09) — after Decision [2026-07-03-0001] removed the previous six. Decision [2026-07-10-0002] answers this: (a) the wildcards are drift and must be removed on the reconciling release PR; (b) doc-automation must base and target `develop` per Decision [2026-07-07-0002]; (c) the initial `develop → main` reconciliation is human-authored this cycle, then a weekly scheduled routine takes over per §7. Implementation sub-tasks still open: **(i)** author the human-driven `develop → main` release PR that resolves the diverged governance-doc state (later timeline from `develop` wins) and strips the two wildcards from `.github/workflows/require-develop-source.yml`; **(ii)** identify and retarget the automation trigger that produced the direct-to-main branches on 2026-07-07 and 2026-07-09 (GitHub Action config, scheduled agent, or external bot) so future runs base off `develop`; **(iii)** enable the weekly scheduled `develop → main` release routine. *Status 2026-07-12:* the drift is still active and growing — `main` merged another wildcard-path doc PR (#281, 2026-07-10) and a fresh main-based bot branch (`doc-requirements-uplift-2026-07-12-*`) appeared on 2026-07-12 re-raising three items `develop` already tracks or has verified as non-drift (see Decision [2026-07-12-0001]); sub-task (ii) remains the root-cause fix.
