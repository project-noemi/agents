...
- **Heading Inconsistency**: The mandatory Diligence heading is "Rules & Constraints" in agents but "Rules & Constraints (4D Diligence)" in skills, complicating automated audit logic. Additionally, 90% of agent personas (e.g., `linux.md`, `student-success-coach.md`) use `## Rules & Constraints (4D Diligence)` headings, creating a documentation drift from the `AGENTS.md` mandate for `## Rules & Constraints`.
