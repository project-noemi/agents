...
- **Value Lens and Operating Profile Injection Drift**: The context generation pipeline (`scripts/generate_all.js`) currently lacks the injection markers and logic to include Value Lenses and Operating Profiles in the generated `GEMINI.md` and `CLAUDE.md` files, despite their status as core architectural requirements.
