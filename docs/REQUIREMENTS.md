...
- **Test Framework Divergence (Executive Assistant)**: `tools/executive-assistant/` uses `vitest`/`supertest` rather than the mandated built-in Node test runner (Requirement §9). Its suite is not globbed by the root `npm test` (`tests/*.test.js`), so it is excluded from the `npm run validate` gate (Clarification [2026-06-12]).
- **Undocumented Reference Service (Executive Assistant)**: `tools/executive-assistant/` is a complete Cloud Run / Gmail Pub/Sub triage service (webhook handler, `TriageRouter`, `OAuthManager`, and admin REST API) but `REQUIREMENTS.md` references it only as an observability gap; its runtime model, endpoints, and triage/confidence model are undocumented, drifting from the §8 "tell the truth" mandate (Clarification [2026-06-12]).
- **Unauthenticated Admin Mutations (Executive Assistant)**: `tools/executive-assistant/server.js` exposes mutating endpoints (`PUT /api/rules`, `POST /api/resolution`) with no authentication, HMAC, or identity validation, contradicting the §7 governance posture and the HMAC/Casdoor reference trust path (Clarification [2026-06-12]).
