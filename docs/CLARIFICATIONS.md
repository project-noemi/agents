...
### ❓ Question [2026-07-03] - Fleet Report Schema: Skill Contract vs. Reference Pipeline
**Context:** The requirements state that reference examples "must tell the truth," and `skills/reporting/structured-report.md` mandates a JSON report with `cycle_timestamp`, `summary.total_evaluated`, nested `summary.actions.*`, and a `details[]` array, explicitly claiming it follows "the Fleet Dashboard ingestion schema". But the codebase in `examples/gatekeeper-deployment/entrypoint.sh` emits `cycle_completed_at`, `repos_scanned`, `prs_evaluated`, and a flat `summary.*` — the shape `dashboard-ingest.js` actually reads.
**Ambiguity / Drift:** An agent that composes the `structured-report` skill as specified would have its reports accepted (HTTP 202) but silently recorded as all-zero metrics, because `buildInfluxLine()` defaults every unrecognized field to `0`/`unknown`. The fleet's flagship example is internally inconsistent: the Gatekeeper persona references `reporting/structured-report`, yet its own reference implementation uses a different schema.
**Question for Product Owner:** Which schema is canonical — the skill's (`cycle_timestamp` + nested `actions`) or the pipeline's (`cycle_completed_at` + flat summary)? Should the losing side be updated in lockstep, and should `dashboard-ingest.js` reject (400) reports missing mandatory fields instead of zero-filling them?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Align the Fleet Dashboard report schema across `skills/reporting/structured-report.md`, `examples/gatekeeper-deployment/entrypoint.sh`, and `dashboard-ingest.js` (including a 400 rejection for reports missing mandatory fields), using the schema the Product Owner designates as canonical.*

### ❓ Question [2026-07-03] - Slack Block Kit Mandate vs. Plain-Text Webhook Summary
**Context:** The Slack MCP protocol mandates "all Slack notifications are properly formatted utilizing Slack's Block Kit," and `skills/reporting/alert-notify.md` requires Block Kit with severity levels and an agent-ID/timestamp footer. The codebase in `examples/gatekeeper-deployment/entrypoint.sh` (`post_slack_summary()`) posts a plain `{"text": "..."}` payload with none of these elements, even though the Gatekeeper persona references `reporting/alert-notify` for this step.
**Ambiguity / Drift:** Builders copying the reference stack will inherit non-compliant Slack notifications, undermining the protocol's alert-fatigue and severity-routing goals. Incoming webhooks do support a `blocks` array, so the gap is implementation drift rather than a platform limitation.
**Question for Product Owner:** Should `post_slack_summary()` be upgraded to emit a Block Kit payload (severity, summary section, agent-ID/timestamp footer context block) per the `alert-notify` skill, or should the skill and Slack protocol be relaxed to permit plain-text summaries for low-severity cycle reports?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Update `post_slack_summary()` in `examples/gatekeeper-deployment/entrypoint.sh` to emit a Block Kit payload with severity, summary section, and an agent-ID/timestamp context footer, per the `alert-notify` skill specification.*

### ❓ Question [2026-07-03] - Timing-Safe Comparison Consistency in Dashboard Ingest
**Context:** The requirements designate the Gatekeeper deployment as the reference for HMAC-signed dashboard submissions. The codebase in `examples/gatekeeper-deployment/dashboard-ingest.js` correctly uses `crypto.timingSafeEqual` for the HMAC signature, but validates the `Authorization: Bearer` token with a plain `!==` string comparison.
**Ambiguity / Drift:** The mixed hardening posture is inconsistent for a security reference implementation: the same request is protected by one timing-safe check and one timing-sensitive check. Practical exploitability over a network is low, but builders copy reference code verbatim, propagating the weaker pattern into production ingest services.
**Question for Product Owner:** Should the Bearer token comparison be hardened to a constant-time check (e.g., `timingSafeEqual` over equal-length hashes of the tokens) so the reference service demonstrates a uniform security posture?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Harden the Bearer token validation in `examples/gatekeeper-deployment/dashboard-ingest.js` to use a constant-time comparison consistent with the existing HMAC verification.*
