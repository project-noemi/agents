...
### ❓ Question [2026-07-05] - Merge Gate Branch Prefix Allowlist Drift
**Context:** Requirement §7 mandates the canonical `develop → main` flow, and `.github/workflows/require-develop-source.yml` enforces it by rejecting PRs to `main` unless the source branch is `develop` or matches one of six hardcoded `doc-*` prefixes (e.g., `doc-requirements-uplift*`).
**Ambiguity / Drift:** The repository's automation now runs on differently-named branches (e.g., `claude/*` session branches, `sync/upstream-*` from Decision [2026-06-30-0001]), whose PRs to `main` fail this gate even when they carry the same governed documentation updates. The allowlist grows only by manual workflow edits, and neither the allowlist nor its maintenance policy is documented in `REQUIREMENTS.md`.
**Question for Product Owner:** Should the merge gate be generalized (e.g., add `claude/*` and `sync/upstream-*` patterns, or switch to a label/actor-based exemption for governed automation), and should the exemption policy be documented in Requirement §7?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Update `.github/workflows/require-develop-source.yml` to exempt governed automation branch patterns (`claude/*`, `sync/upstream-*`) per the approved policy and document the merge-gate exemption rules in `REQUIREMENTS.md` §7.*
