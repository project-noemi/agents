...
if ($Mode -eq "docker") {
    if (-not $secretsCli) {
        Write-Host "`n❌ Docker mode requires at least one SecretOps CLI (1Password or Infisical)." -ForegroundColor Red
        Write-Host "   Install one and re-run before proceeding to runtime deployment." -ForegroundColor Yellow
        exit 1
    }
    if (-not $secretsAuth) {
        Write-Host "`n❌ Docker mode requires active SecretOps authentication." -ForegroundColor Red
        Write-Host "   Run 'op signin' or 'infisical login' and re-run this script." -ForegroundColor Yellow
        exit 1
    }
}
