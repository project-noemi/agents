...
### 11. Licensing and Distribution Transparency

- The repository is distributed as **Fair Source** software under the **Functional Source License, FSL-1.1-Apache-2.0** (Decision [2026-07-05-0004]): each published version is freely readable, usable, modifiable, and redistributable, with a two-year restriction on offering a competing commercial NoéMI-as-a-service, after which that version automatically converts to Apache-2.0.
- `package.json` must declare the SPDX `license` identifier matching `LICENSE`.
- **Tagged GitHub releases are the canonical published versions** for the two-year conversion timeline; a release tag must exist at or after the relicense merge so the first conversion clock has an unambiguous start date.
- The **NoéMI Transparent™** mark and its five-clause guarantee are defined in [`TRANSPARENT_SOURCE.md`](TRANSPARENT_SOURCE.md) (Public Specs, Verifiable Context, Structured Auditing, Public Governance, Deployment Disclosure). Legal rights flow from the FSL; trust norms flow from the mark. Repository changes must not break the mechanically enforced clauses (deterministic context generation, golden fixtures, mandatory persona Audit Log contracts, public decision log).
- Self-referential documentation must describe the project as "Fair Source," not "open source"; references to third-party open-source components are unaffected.
...
- **CI/CD "False Green" Risk in E2E**: `tests/e2e/docker-smoke.test.js` silently skips runtime checks if Docker is absent. Decision [2026-07-05-0006] approved an opt-in `FORCE_DOCKER_SMOKE=true` mandatory-mode flag; implementation pending — the test currently has no `FORCE_DOCKER_SMOKE` branch.
...
- **AI Model Baseline Drift (Sovereign Pins)**: `operating-profiles/local-sovereign-profile.json` pins `deepseek-coder:67b-instruct` and `llama3.3:70b-instruct`, which the single-baseline mandate does not currently recognize (see Clarification [2026-07-05] Sovereign Model Pins). The legacy-example portion was remediated on 2026-07-05: `examples/docker/agent.py` now uses `gemini-2.5-flash` (Decision [2026-07-05-0008]).
...
- **Merge Gate Wildcard Regression on `main`**: commit `a0a7fad` (`google-labs-jules[bot]`, 2026-07-06, no decision entry) re-added a `^doc-requirements-uplift-` wildcard to `.github/workflows/require-develop-source.yml` on `main`, contradicting Requirement §7 and Decisions [2026-07-03-0001]/[2026-07-07-0002], and allowed PR #273 to merge straight to `main` (renewing `main`/`develop` divergence). The strict gate was restored on the automation branch in the 2026-07-09 sync-down (Decision [2026-07-09-0001]); `main` retains the wildcard until the next release PR, and the Jules doc-uplift routine still needs repointing at `develop` (see Clarification [2026-07-09] Merge Gate Wildcard Regression).
- **FSL Conversion Clock Unstarted**: Requirement §11 and Decision [2026-07-05-0004] make tagged releases the canonical published versions for the two-year FSL→Apache-2.0 conversion timeline, and mandate a tag at the relicense merge. The only existing tag, `v0.1.0` (commit `4d74c8a`), pre-dates the relicense and still carries the SSPL `LICENSE`; no FSL-era tag exists, so the first conversion clock has no unambiguous start date (see Clarification [2026-07-09] FSL Release Tagging).
...
- **Phase 0 Audit Gap**: `scripts/audit-repo.js` lacks logic to verify the presence of the 8+ mandated assessment files in `docs/phase-zero-assessment/`, violating Requirement §1. Decision [2026-07-05-0011] approved adding a file-presence check that fails the audit when any Phase 0 kit file is missing; implementation pending.
...
- **Operating Profile Starter Set Incomplete**: of the three-profile baseline approved by Decision [2026-06-19-0011], only `operating-profiles/standard-operating-profile.md` has shipped (commit `9c19548`); `high-trust-regulated.md` and `fast-iteration-startup.md` remain unimplemented. The service tier templates portion of the former "Infrastructure Asset Implementation Gap" was remediated in the same commit (see Archive).
- **Ungoverned JSON-only Value Lens**: `value-lenses/compassion-lens.json` defines a fleet-wide lens (adjustments, refusal criteria) with no Markdown counterpart, no registration in `value-lenses/README.md` or `docs/frameworks/value-lenses.md`, and no injection into generated context (the generator only reads `.md` lens files). It is the value-lens instance of the second-format governance gap; the canonical dual-format pattern is `american-dream.md` + `.json` companion (see Clarification [2026-07-09] Compassion Lens JSON-only Governance).
...
- **Red Team JSON Companion Drift**: `examples/red-team-gauntlet/test-vectors.json` (salvaged in commit `9c19548`) coexists with the canonical `test-vectors.yaml` even though Decision [2026-07-07-0003] — written without knowledge of the salvaged file — declared YAML canonical. The JSON file's header instructs "keep both in lockstep" with no enforcement, and its `$schema` field points at `./test-vectors.schema.json`, which does not exist. Small implementation task: either delete the JSON file per the canonical-YAML decision or ship the missing schema and a lockstep check.

### 5. Remediated Limitations (Archive)
- **Service Tier Templates and Standard Operating Profile** (Remediated: 2026-07-05, verified 2026-07-09): `templates/tiers/basic.md`, `standard.md`, and `premium.md` now exist with substantive content (no TBD placeholders), and `operating-profiles/standard-operating-profile.md` fills the previously empty operating-profile injection block in generated context (commit `9c19548`, Decision [2026-07-09-0002]). The remaining two approved profiles are tracked under "Operating Profile Starter Set Incomplete."
...
- **ROI Auditor Baseline Access Ambiguity** (Resolved: 2026-07-07): the committed `tools/roi/baseline-config.json` snapshot path from Decision [2026-06-19-0015] is canonical per Decision [2026-07-07-0004]; the live-Sheets alternative in Decision [2026-06-12-0001] is annotated as superseded. Implementation still pending — the file itself does not yet exist.
