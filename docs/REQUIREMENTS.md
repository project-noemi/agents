...
- **Structural vs. Substantive Compliance**: The 5 coding-domain agents (`architect/core.md`, `bolt/core.md`, `bolt/go.md`, `bolt/nextjs-16.md`, `sentinel/core.md`) now provide role-specific `Data Inventory` and `Refusal Criteria` content. The remaining 21 personas across 8 other domains continue to use identical placeholder text, satisfying structural audits but failing substantive 4D Description (D2) and Refusal Principle requirements.
...
- **Pre-flight Logic Contradiction**: `scripts/verify-env.sh` previously contained redundant and contradictory SecretOps checks — a hard-fail block followed by a separate warning block. The script now uses a consistent warning-only approach: missing SecretOps CLIs emit a `⚠️` notice but do not set `ALL_GOOD=false` or trigger `exit 1` (Resolved: 2026-05-26).
...
- **Agent Journal Directory Gap**: The 5 coding-domain agents (`architect/core.md`, `bolt/core.md`, `bolt/go.md`, `bolt/nextjs-16.md`, `sentinel/core.md`) reference a `.jules/` directory for Journal entries (e.g., `.jules/architect.md`, `.jules/bolt-go.md`). This directory does not exist in the repository and the `Journal` section pattern is not documented in `docs/AGENT_TEMPLATE.md` or standardized in `AGENTS.md`.
- **Bolt Next.js-16 Node.js Version Drift**: `agents/coding/bolt/nextjs-16.md` specifies "Node.js (v22+)" in its `External Tooling Dependencies` section, which conflicts with the repository's mandatory Node.js 24 baseline (see Runtime and Tooling Requirements and AGENTS.md Coding Standards).
