...
- **Skill Contract Substantive Drift**: Reusable skills in `skills/` often contain "TBD" placeholders for mandatory `Data Inventory` and `Refusal Criteria` sections, failing substantive compliance with the 4D framework.
- **Audit Script Enforcement Depth** (Remediated: 2026-05-31): `scripts/audit-repo.js` now verifies referential integrity for `mcp.config.json` and includes mandatory branch protection checks (Decision [2026-05-31-0002]).
- **Agent Index Accuracy Drift** (Remediated: 2026-05-31): `scripts/context_helpers.js` now extracts the full first paragraph of the `Role` section for richer Agent Index generation (Decision [2026-05-31-0003]).
- **Internal Tool Observability Gap** (Remediated: 2026-05-31): Repository build tools and internal reference services now emit structured JSON Audit Logs to `stderr` via the shared `scripts/audit_logger.js` utility (Decision [2026-05-31-0001]).
- **Docker Smoke Test Variable Validation Gap** (Remediated: 2026-05-31): `tests/examples-smoke.test.js` now validates that `NOEMI_DOCKER_SMOKE_*` variables are defined in `.env.template` (Decision [2026-05-31-0004]).
- **Sync Script Parameterization Gap** (Remediated: 2026-05-31): `scripts/sync-upstream.sh` now uses environment variables for organization name and upstream URL (Decision [2026-05-31-0005]).
