...
### ❓ Question [2026-06-04] - Docker Smoke Artifact Directory Drift (.env.template vs. test default)
**Context:** Requirement §9 names `.env.template` the single source of truth for the `NOEMI_DOCKER_SMOKE_*` variables consumed by `tests/e2e/docker-smoke.test.js`. The test reads `NOEMI_DOCKER_SMOKE_ARTIFACT_DIR` (default `test-artifacts/docker-smoke`), `NOEMI_DOCKER_SMOKE_TIMEOUT_MS` (default `600000`), and `NOEMI_DOCKER_SMOKE_POLL_INTERVAL_MS` (default `2000`).
**Ambiguity / Drift:** The values in `.env.template` disagree with the test's own fallbacks: `NOEMI_DOCKER_SMOKE_ARTIFACT_DIR=./tests/e2e/artifacts` (test falls back to `test-artifacts/docker-smoke`), `TIMEOUT_MS=60000` (test default `600000` — a 10× difference), and `POLL_INTERVAL_MS=5000` (test default `2000`). Critically, `.gitignore` ignores `test-artifacts/` but **not** `tests/e2e/artifacts/`, so a builder who runs the suite via `op run --env-file=.env.template` writes Docker container logs and runtime artifacts into a tracked path — risking accidental commit of runtime output (and any sensitive log content) into the repository.
**Question for Product Owner:** Which value is canonical — should `.env.template` be corrected to match the test defaults (`test-artifacts/docker-smoke`, `600000`, `2000`), or should the test defaults and `.gitignore` be updated to match `.env.template` (`./tests/e2e/artifacts`)? Either way the artifact directory must be a gitignored path.
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Reconcile the `NOEMI_DOCKER_SMOKE_*` values between `.env.template` and `tests/e2e/docker-smoke.test.js`, and ensure the chosen `NOEMI_DOCKER_SMOKE_ARTIFACT_DIR` path is covered by `.gitignore`.*

### ❓ Question [2026-06-04] - Decision Log References a Non-Existent Fleet Dashboard Path
**Context:** Decision [2026-05-20] (Fleet Dashboard Ingestion Path) states the reference implementation lives at `examples/fleet-dashboard/dashboard-ingest.js` and mandates updating it in lockstep with `tests/examples-smoke.test.js`.
**Ambiguity / Drift:** No `examples/fleet-dashboard/` directory exists. The actual ingest service is `examples/gatekeeper-deployment/dashboard-ingest.js`, and the Fleet Dashboard persona (`agents/operations/fleet-dashboard.md`) correctly points to `examples/gatekeeper-deployment/`. The durable decision record therefore cites a path that does not exist, weakening the audit trail and risking a future contributor creating a duplicate stub at the phantom path.
**Question for Product Owner:** Should the Fleet Dashboard reference implementation be relocated/split into its own `examples/fleet-dashboard/` directory (matching the decision and the persona's logical name), or should the decision record and any lingering references be corrected to point at the canonical `examples/gatekeeper-deployment/dashboard-ingest.js`?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Reconcile the Fleet Dashboard reference-implementation path: either move `dashboard-ingest.js` into `examples/fleet-dashboard/` or correct the path cited in `DECISION_LOG.md` [2026-05-20] to `examples/gatekeeper-deployment/dashboard-ingest.js`.*

### ❓ Question [2026-06-04] - Resilience Helper Limitation Contradicts a Durable Decision
**Context:** `REQUIREMENTS.md` Current Known Limitations lists a "Resilience Helper Integration Gap," asserting that `scripts/resilience_helpers.js` "is not utilized by repository tools (`audit-repo.js`, `generate_all.js`) or any agent personas, **violating the resilience mandate**."
**Ambiguity / Drift:** This characterization contradicts Decision [2026-04-04] (Resilience Helpers: Core Script Integration Scope), which explicitly resolved that `resilience_helpers.js` satisfies the mandate **as a reference pattern** and must **not** be force-integrated into `audit-repo.js`/`generate_all.js` because those perform deterministic local filesystem operations that do not benefit from backoff. A verification sweep confirms the helper is referenced only by itself, so the factual "unused by core scripts" observation is accurate — but framing it as a *violation* mischaracterizes a settled decision and makes the requirements doc internally inconsistent.
**Question for Product Owner:** Should this limitation be closed (re-worded to note resilience_helpers.js is intentionally reference-only per Decision [2026-04-04]), or has the policy changed such that core scripts/agents should now actively consume it?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Update the "Resilience Helper Integration Gap" entry in `REQUIREMENTS.md` to reflect Decision [2026-04-04] (reference-only is compliant), removing the "violating the resilience mandate" framing.*
