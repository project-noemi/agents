...
### ❓ Question [2026-05-25] - REQUIREMENTS.md Stale Known Limitations (Inverse Drift)
**Context:** The requirements state that "Pre-flight Script Shallow Validation: `scripts/verify-env.sh` and `scripts/verify-env.ps1` check for CLI tool presence but lack active authentication verification" and that a "Pre-flight Logic Contradiction" exists where "one block treats the absence of `infisical` or `op` as a hard failure (exit 1), while a subsequent block treats it as a warning." A whole-codebase verification of `scripts/verify-env.sh` (lines 139–163) and `scripts/verify-env.ps1` (lines 98–123) confirms that both scripts now include active `op user get --me` and `infisical whoami` authentication checks, and that the SecretOps section uses a single, consistent warning-only path with no `ALL_GOOD=false` assignment.
**Ambiguity / Drift:** The "Current Known Limitations" section of `REQUIREMENTS.md` lists two items as open that the codebase has already resolved. Keeping stale limitations listed as open creates false signals about the repository's compliance health, potentially causing contributors to investigate non-existent issues or skip real work.
**Question for Product Owner:** Should these two resolved limitations be removed from `REQUIREMENTS.md`, and should a lightweight "resolved limitations" registry be maintained in `DECISION_LOG.md` to prevent future documentation-over-code drift?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Remove the "Pre-flight Script Shallow Validation" and "Pre-flight Logic Contradiction" entries from `REQUIREMENTS.md` Current Known Limitations and add a `[2026-05-25] Pre-flight Scripts: Authentication and Consistency Remediation` entry to `DECISION_LOG.md` confirming resolution.*

### ❓ Question [2026-05-25] - Decision [2026-05-20] Partial Implementation in `audit-repo.js`
**Context:** The requirements state (`REQUIREMENTS.md` Section 3) that "`Audit Log` sections contain structurally invalid JSON (Mandatory JSON shape validation)" must trigger a failed audit. `DECISION_LOG.md` [2026-05-20] explicitly mandated that `scripts/audit-repo.js` must: (1) audit the `skills/` directory ✅ implemented; (2) validate the Audit Log JSON against the canonical schema (`{ "task": "...", "inputs": [], "actions": [], "risks": [], "result": "..." }`) ❌ not implemented; (3) emit its own structured JSON Audit Log to `stderr` ❌ not implemented. A code inspection of `scripts/audit-repo.js` (lines 77–94) confirms only `JSON.parse()` is called — there is no key validation against the required schema.
**Ambiguity / Drift:** `REQUIREMENTS.md` Section 3 implies "Mandatory JSON shape validation" is enforced, but the current gate only verifies parseability. An Audit Log containing `{}` or `{"foo":"bar"}` passes the audit. This is a compliance gap with a *completed* architectural decision, not an open question.
**Question for Product Owner:** Should Jules be immediately tasked with completing the Decision [2026-05-20] mandate by implementing key-set validation (`task`, `inputs`, `actions`, `risks`, `result`) and adding `stderr` Audit Log emission to `audit-repo.js`?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Enhance `scripts/audit-repo.js` to (1) validate that each Audit Log JSON object contains all five required keys and (2) emit a structured JSON summary of audit results to `stderr` upon completion, fulfilling both unimplemented mandates from Decision [2026-05-20].*

### ❓ Question [2026-05-25] - `SKILL_TEMPLATE.md` Structural Non-Compliance
**Context:** `skills/SKILL_TEMPLATE.md` is the canonical starting point for creating new reusable skills, and `REQUIREMENTS.md` Section 2 mandates that all skills include `## Data Inventory` and a `### Refusal Criteria` H3 subsection under `Rules & Constraints`. A direct inspection of `skills/SKILL_TEMPLATE.md` confirms it contains neither `## Data Inventory` nor `### Refusal Criteria` (it has `## Rules & Constraints (4D Diligence)` but no `Refusal Criteria` subsection). Additionally, the template's `## Audit Log` section uses raw unformatted JSON rather than a ` ```json ``` ` fenced code block. The structural audit at `scripts/audit-repo.js` line 139 explicitly skips the template file (`file !== 'SKILL_TEMPLATE.md'`), so this gap is invisible to the CI gate.
**Ambiguity / Drift:** Every new skill scaffolded from this template will be born structurally non-compliant and will fail the skill audit on first commit. This is a "factory defect" that multiplies technical debt with each new skill added.
**Question for Product Owner:** Should `SKILL_TEMPLATE.md` be updated immediately to include `## Data Inventory` and `### Refusal Criteria` placeholders, and should the audit be extended to validate the template file as well to prevent regression?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Update `skills/SKILL_TEMPLATE.md` to add a `## Data Inventory` section between `Outputs` and `Rules & Constraints`, add a `### Refusal Criteria` H3 subsection inside `Rules & Constraints (4D Diligence)`, and wrap the Audit Log in a ` ```json ``` ` fenced code block.*
