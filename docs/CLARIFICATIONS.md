...
- [2026-07-09] Reconciliation pass (Decisions [2026-07-09-0001] and [2026-07-09-0002]): PR #273's three re-added questions (Automation Branch Flow, TBD Placeholder Rejection, Agent Index Regex) were dropped during the main→develop sync-down as duplicates of already-decided items ([2026-07-07-0002], the [2026-05-20]/[2026-06-19-0005] audit-coverage family, and [2026-06-19-0012] respectively). Q [2026-06-10] Empty Tier Templates and Q [2026-06-15] Service Tier Template Specifications closed as overtaken by events — `templates/tiers/{basic,standard,premium}.md` shipped in commit `9c19548` along with `operating-profiles/standard-operating-profile.md`. Backlog shrinks from 15 to 13 open questions, plus three new questions dated 2026-07-09 (FSL release tagging / Transparent Source Guarantee enforcement, Compassion Lens JSON-only governance, merge-gate wildcard regression).
...
### ❓ Question [2026-07-09] - FSL Release Tagging and Transparent Source Guarantee Enforcement
**Context:** Decision [2026-07-05-0004] relicensed the repository to FSL-1.1-Apache-2.0 and established the five-clause NoéMI Transparent™ guarantee in `docs/TRANSPARENT_SOURCE.md`, stating that "tagged GitHub releases are the canonical published versions" for the two-year Apache-2.0 conversion timeline and that "a tagged release is to be cut at the relicense merge commit." The only existing tag, `v0.1.0`, points at commit `4d74c8a`, which pre-dates the relicense and still contains the SSPL `LICENSE` file.
**Ambiguity / Drift:** No FSL-era tag exists, so the first conversion clock has no legally unambiguous start date — the central mechanism of the license's "delayed open source" promise is currently unstarted. Separately, the guarantee's five clauses are enforced unevenly: clause 2 (Verifiable Context) is CI-enforced via golden fixtures, but clauses 1, 3, and 4 have no corresponding checks in `scripts/audit-repo.js`, and `REQUIREMENTS.md` did not reference the guarantee at all until this pass added §11.
**Question for Product Owner:** Should a release tag (e.g., `v0.2.0`) be cut at or after the relicense merge to start the FSL conversion clock, and should the mechanically checkable clauses of the Transparent Source Guarantee (no hidden prompt files, audit-log contract presence, decision-log currency) be added to the `scripts/audit-repo.js` gate?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Cut the post-relicense release tag and extend `scripts/audit-repo.js` with checks for the mechanically verifiable Transparent Source Guarantee clauses defined in `docs/TRANSPARENT_SOURCE.md`.*

### ❓ Question [2026-07-09] - Compassion Lens JSON-only Governance
**Context:** `value-lenses/compassion-lens.json` (added in commit `bb381d2`) defines a fleet-wide "compassion lens" with discernment/diligence adjustments and refusal criteria, but it has no Markdown counterpart, is not registered in `value-lenses/README.md` or `docs/frameworks/value-lenses.md`, and is invisible to the Markdown-only generator (`scripts/context_helpers.js` filters `.endsWith('.md')`), so it is never injected into `GEMINI.md`/`CLAUDE.md`. By contrast, the contributed `american-dream` lens ships as a Markdown lens with a JSON companion (Decision [2026-07-05-0003]) — the canonical dual-format pattern.
**Ambiguity / Drift:** A value lens that claims to "enforce the Anti-Replacement rule across the entire fleet" exists in the repository but is unreachable by any agent, unaudited, and unregistered — the same second-format governance gap as the sovereign JSON assets (Clarification [2026-07-05] Sovereign JSON Asset Layer Governance), but in the value-lens layer, which that question does not cover.
**Question for Product Owner:** Should `compassion-lens.json` be promoted to a canonical Markdown lens with a JSON companion (following the `american-dream` pattern) and registered in the lens READMEs, or should it be retired/folded into the sovereign JSON governance decision?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Author `value-lenses/compassion-lens.md` from the JSON definition following the `american-dream.md` structure, register it in `value-lenses/README.md` and `docs/frameworks/value-lenses.md`, and regenerate the context files.*

### ❓ Question [2026-07-09] - Merge Gate Wildcard Regression and Doc-Uplift Routine Repointing
**Context:** Requirement §7 and Decisions [2026-07-03-0001]/[2026-07-07-0002] make `develop` the only valid PR source into `main` and prohibit broad wildcard exceptions in `.github/workflows/require-develop-source.yml`. On 2026-07-06, commit `a0a7fad` (authored by `google-labs-jules[bot]`, no decision entry) re-added a `^doc-requirements-uplift-` wildcard to the gate on `main` to "fix CI deadlock," which let PR #273 merge straight to `main` and diverged `main` from `develop` again — the exact cycle the decision was written to eliminate. The 2026-07-09 sync-down merge restored the strict gate on the automation branch; `main` retains the wildcard until the next release PR.
**Ambiguity / Drift:** The Jules doc-uplift routine is evidently still configured to open PRs from `doc-requirements-uplift-*` branches against `main`. With the strict gate restored, its next run will fail the gate by design. The durable fix is outside this repository: the routine's scheduler configuration must be repointed at `develop`.
**Question for Product Owner:** Will you repoint the Jules doc-uplift routine to target `develop` (per Requirement §7), or should a single explicit, documented exception be recorded in the decision log and the gate instead?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *Confirm the doc-uplift scheduler now targets `develop`, then remove any remaining wildcard from `.github/workflows/require-develop-source.yml` on all branches and record the closure in the decision log.*
