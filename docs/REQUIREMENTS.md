...
- **Dashboard Ingest Persistence Backend Undocumented**: Requirement §8 documents only the front-door of the Fleet Dashboard ingest path (HMAC-signed submissions to `/api/v1/reports`). The reference implementation `examples/gatekeeper-deployment/dashboard-ingest.js` additionally persists each report to InfluxDB via line protocol (the `cycle_report` measurement, tagged by `agent_id` and `org`, with the integer fields `repos_scanned`, `pr_count`, `auto_merged`, `flagged_for_review`, `closed_stale_conflict`, `skipped`, `errors` and a `mode` string), configured by the undocumented `INFLUXDB_WRITE_URL` and `INFLUXDB_AUTH_TOKEN` environment variables. The persistence backend and its measurement schema are not reflected in the requirements (open clarification [2026-06-11]).
- **Backoff Parameter Drift Between Reference Implementations**: Requirement §8 names `scripts/resilience_helpers.js` as the canonical exponential-backoff pattern but specifies no parameter values. The Node helper uses `maxRetries=5`, `baseDelayMs=1000`, `maxDelayMs=30000`, and `jitterFactor=0.2`, while the shell `retry_with_backoff` in `examples/gatekeeper-deployment/entrypoint.sh` defaults to 5 attempts, a 1 s base, ×2 growth, a **16 s** cap, and **no jitter**. The two reference implementations diverge on the delay ceiling and jitter, and no canonical parameter set is documented (open clarification [2026-06-11]).
- **Dashboard Ingest HTTP/Error Contract Undocumented**: The requirements name the `/api/v1/reports` path but do not specify its response semantics. `dashboard-ingest.js` returns `202 Accepted` on success, `400` on malformed JSON, `401` on a missing Bearer token or invalid HMAC signature, `404` for unknown routes, `502` when InfluxDB persistence fails, and exposes `GET /health` → `200`. Correspondingly, the client `entrypoint.sh` treats `401` as fatal (no retry) while retrying `429`/`5xx`/network failures. This response and retry-classification contract is implemented but undocumented (open clarification [2026-06-11]).
