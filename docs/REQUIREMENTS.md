...
- **Substantive Skill Compliance**: All 26 agent personas now include role-specific, substantive content for `Data Inventory` and `Refusal Criteria` (Resolved for agents: 2026-05-27). However, all 8 reusable skills still use `TBD` placeholder text for these sections, and `SKILL_TEMPLATE.md` does not include a `## Data Inventory` section despite `scripts/audit-repo.js` now enforcing it as a mandatory skill heading, creating a contributor-facing audit failure trap.
...
- **Pre-flight Logic Contradiction**: The contradictory hard-failure block was removed from `scripts/verify-env.sh`; the script now uses a single unified warning block for missing SecretOps CLIs, with active authentication checks (`op user get --me`, `infisical whoami`) executed as non-blocking advisories (Resolved: 2026-05-27).
- **Coding Agent Documentation Drift**: `docs/agents/coding/README.md` lists only Bolt and Sentinel personas; the Architect agent (added 2026-05-24) is not documented in that index.
