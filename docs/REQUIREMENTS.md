...
- **Pre-flight Logic Contradiction**: `scripts/verify-env.sh` has been remediated to remove redundant hard-failure blocks for missing SecretOps CLIs, standardizing on a warning for local exploration (Resolved: 2026-05-26).
- **Skill Contract Structural Alignment**: All 8 reusable skills now include the mandatory `## Data Inventory` section and the `### Refusal Criteria` H3 subsection, although content remains as placeholders (Resolved: 2026-05-22). However, `SKILL_TEMPLATE.md` remains un-remediated.
...
- **Template Marker Duplication**: `templates/context/GEMINI.template.md` and `CLAUDE.template.md` have been verified as NOT containing duplicate marker pairs (Resolved: 2026-05-26).
