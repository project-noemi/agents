...
### ❓ Question [2026-05-30] - Golden Fixtures Gap for Framework Injection Markers
**Context:** Decision [2026-05-28-0005] mandated that `scripts/generate_all.js` inject `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS` into the generated context files. `scripts/context_helpers.js` lists both as required template markers, and both templates carry the correct marker pairs. However, `tests/golden-fixtures.test.js` only covers four markers (`GLOBAL_MANDATES`, `AGENT_INDEX`, `SKILLS_INJECTIONS`, `MCP_INJECTIONS`) and has no corresponding fixture files in `tests/fixtures/generated/` for the two framework markers.
**Ambiguity / Drift:** The injector for Value Lens and Operating Profile content has no CI regression gate. A change to `scripts/generate_all.js`, `value-lenses/`, or `operating-profiles/` that silently corrupts these sections would not be caught by `npm test`. This contradicts Requirement §9's mandate for "golden fixtures for generated context sections."
**Question for Product Owner:** Should we run `scripts/update-golden-fixtures.js` to snapshot the current output for `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS`, and extend `tests/golden-fixtures.test.js` to include these two markers?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Extend `scripts/update-golden-fixtures.js` to generate fixture files for `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS`, add corresponding entries to the fixtures array in `tests/golden-fixtures.test.js`, and run `npm run test:update-fixtures` to populate the initial snapshots.*

### ❓ Question [2026-05-30] - GMU Validation Example Governance and Documentation Gap
**Context:** `examples/gmu-validation/verification-bot.js` is a Node.js script that simulates a student completion audit for a George Mason University (GMU) Boot Camp milestone (Phase 0 Security + Red Team Gauntlet). It contains mock execution log data, a `verifyStudent()` function, and console output. The directory contains no README and is not referenced anywhere in REQUIREMENTS.md, DECISION_LOG.md, or the main onboarding docs.
**Ambiguity / Drift:** The repository's Examples section defines four governed example topologies (docker, fleet-deployment, gatekeeper-deployment, video-automation-pod), but this fifth directory is ungovorned. It is unclear whether it is: (a) a legitimate reference example that should be documented and maintained, (b) a demo/prototype that should carry an ILLUSTRATIVE disclaimer, or (c) out-of-scope and candidate for removal. Without a governance decision, it creates a maintenance liability and may mislead builders exploring the examples directory.
**Question for Product Owner:** Should `examples/gmu-validation/` be formally documented (README + entry in REQUIREMENTS.md) as a reference scenario, labeled ILLUSTRATIVE and kept as a demo artifact, or removed from the repository?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Based on the product owner's decision: (a) add a README to `examples/gmu-validation/` and a documentation entry in REQUIREMENTS.md §8, (b) add an ILLUSTRATIVE comment header to `verification-bot.js` and a brief README flagging its demo status, or (c) remove the directory and update the examples index.*

### ❓ Question [2026-05-30] - Test Coverage Specification Granularity in REQUIREMENTS.md §9
**Context:** Requirement §9 specifies that the default test suite "must cover: persona and template contracts, generator determinism and config override behavior, golden fixtures for generated context sections, and static smoke checks for example stacks and Docker env inventories." This was an accurate description of the test suite when written.
**Ambiguity / Drift:** `tests/contracts.test.js` has since grown to validate 14+ distinct artifact categories not listed in §9: cross-platform kickstart guides (macOS/Linux, Windows, ChromeOS, cross-platform), Microsoft 365 agentic client docs, Google Workspace multi-path setup docs (gws-cli machine setup, generic MCP, n8n quickstart, client guide, credential matrix), local workspace docs (Gemini, Claude, Codex), visual guides with mermaid diagrams (system map, audience map, runtime flow, mind map), n8n anti-hallucination verification (no invented helper APIs), RFP responder workflow Gemini node type pinning, value lens and operating profile framework docs, and Node.js baseline pinning across `.nvmrc`, `.node-version`, CI workflow, and `package.json`. The requirement as written no longer reflects the implemented scope.
**Question for Product Owner:** Should §9 be expanded to enumerate the full set of verified artifact categories (making the requirement actionable for new contributors), or should it be rewritten at a higher abstraction level (e.g., "all artifacts referenced in the front-door docs and generated context must be smoke-verified by the test suite")?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Update `docs/REQUIREMENTS.md` §9 to enumerate the full list of currently verified artifact categories derived from `tests/contracts.test.js` and `tests/examples-smoke.test.js`, or rewrite the section using the proposed abstraction-level language — whichever the product owner selects.*
