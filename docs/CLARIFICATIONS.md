...
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Remove duplicate markers from `templates/context/GEMINI.template.md` and ensure a clean, single-injection structure for mandates and the agent index.*

### ❓ Question [2026-05-26] - Bolt Next.js-16 Node.js Version Drift
**Context:** `REQUIREMENTS.md` and `AGENTS.md` mandate Node.js 24 as the canonical technical baseline for all repository logic, utilities, and agent tooling. The `Runtime and Tooling Requirements` section and the `Coding Standards` section are explicit: "Node.js is the primary runtime" and "All repository logic, utilities, and reference Docker images must use Node.js version 24."
**Ambiguity / Drift:** `agents/coding/bolt/nextjs-16.md` specifies `Node.js (v22+)` in its `## External Tooling Dependencies` section. Since Next.js 16 requires React 19 and may not mandate Node 24 at the framework level, it is unclear whether this v22+ specification is intentional (reflecting the minimum framework requirement) or an oversight that should be updated to v24+ to match the repository baseline.
**Question for Product Owner:** Should `agents/coding/bolt/nextjs-16.md` be updated to specify `Node.js (v24+)` to align with the repository baseline, or should it document v22+ as the minimum framework requirement alongside a note that the NoéMI deployment environment enforces v24?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Update `agents/coding/bolt/nextjs-16.md` `## External Tooling Dependencies` to specify `Node.js (v24+)` (or add a parenthetical note that the NoéMI baseline requires v24 even if the framework minimum is v22), ensuring alignment with the repository-wide baseline mandate.*

### ❓ Question [2026-05-26] - Agent Journal Directory Standardization
**Context:** The 5 coding-domain agents (`architect/core.md`, `bolt/core.md`, `bolt/go.md`, `bolt/nextjs-16.md`, `sentinel/core.md`) each include a `## Journal` section that references a `.jules/` directory for critical learnings (e.g., `.jules/architect.md`, `.jules/bolt-go.md`). This pattern provides a valuable mechanism for cross-cycle learning retention.
**Ambiguity / Drift:** The `.jules/` directory does not exist in the repository. The `Journal` section is not mentioned in `docs/AGENT_TEMPLATE.md` as an optional section, and `AGENTS.md` does not document the pattern. As a result, the section is invisible to new persona authors, inconsistently adopted across the fleet (only 5 of 26 agents), and the journal files would not be created automatically by any workflow.
**Question for Product Owner:** Should the `Journal` section be formally documented as an **optional** section in `docs/AGENT_TEMPLATE.md` and `AGENTS.md` with a canonical directory convention (e.g., `.noemi/journals/<agent-name>.md` rather than the `.jules/`-specific path), and should the `.jules/` directory be initialized with a `.gitignore` to prevent journal file leakage?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Add `Journal` as a documented optional section to `docs/AGENT_TEMPLATE.md` and `AGENTS.md` with a standardized directory convention, initialize the directory with a `.gitignore`, and update the 5 coding-domain agents to use the canonical path.*

### ❓ Question [2026-05-26] - Stale Skills-Audit Questions in CLARIFICATIONS.md
**Context:** A whole-codebase inspection of `scripts/audit-repo.js` confirms that the `checkSkills()` function (lines 113–152) is already implemented and called from `main()`. It discovers all `*.md` files under `skills/`, applies the same `auditFile()` structural checks used for agents, and verifies the `### Refusal Criteria` H3 presence and mandatory JSON Audit Log shape. The repository audit (`node scripts/audit-repo.js`) confirms: *"Audited 8 skills."*
**Ambiguity / Drift:** At least 8 questions in this file (dated 2026-04-25 through 2026-05-20) ask whether `audit-repo.js` should be "expanded" or "enhanced" to audit the `skills/` directory — treating this as an open, unimplemented gap. These questions are now stale: the implementation already exists. Leaving them open risks misleading the Product Owner into authorizing redundant work.
**Question for Product Owner:** Should the following stale questions be formally retired to `DECISION_LOG.md` (marked as "superseded by implemented code") and removed from this active clarifications file: Q [2026-04-25] Skill Contract Audit Enforcement, Q [2026-05-02] Automated Audit Script Coverage Gaps, Q [2026-05-10] Audit Script Coverage Expansion, Q [2026-05-13] Skill Directory Audit Enforcement, Q [2026-05-19] Audit Script JSON Schema Validation, Q [2026-05-20] Audit Script Enforcement Depth?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Action Prompt:** *Archive the listed stale skills-audit questions to `docs/DECISION_LOG.md` as "Resolved by implemented code" and remove them from `docs/CLARIFICATIONS.md` to keep the active backlog accurate and actionable.*
