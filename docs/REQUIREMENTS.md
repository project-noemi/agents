...
- **Skill Library Substantive Remediation Strategy**: 100% of the active reusable skills in `skills/` currently contain "TBD" placeholders for mandatory `Data Inventory` and `Refusal Criteria` sections, failing substantive compliance with the 4D framework.
- **Audit Script Schema Blindness**: `scripts/audit-repo.js` fails to verify that skills or MCPs referenced in `mcp.config.json` and agent workflows exist as physical files. It also lacks schema-level validation for `Audit Log` JSON, performing only basic structural parsing and failing to detect "TBD" placeholders.
...
- **Internal Tool Observability Gap**: Node.js tools in `tools/` (e.g., `executive-assistant/server.js`) and reference services in `examples/` (e.g., `gatekeeper-deployment/dashboard-ingest.js`) lack structured JSON Audit Log emission to `stderr`, relying on unstructured `console.log` and missing the shared `audit_logger.js` utility mentioned in architectural mandates.
- **Docker Smoke Test Variable Validation Gap**: `tests/examples-smoke.test.js` lacks validation for mandated `NOEMI_DOCKER_SMOKE_*` environment variables found in `.env.template`.
...
- **Memory-Code Synchronization Drift**: Architectural remediations recorded as completed on 2026-06-06 (sync script parameterization, agent index paragraph extraction, and `audit_logger.js`) are absent or reverted in the codebase.
