...
- **Framework Injection Gap**: `Value Lenses` and `Operating Profiles` are documented in `docs/frameworks/` but not yet injected by `scripts/generate_all.js` although `VALUE_LENS_INJECTIONS` and `OPERATING_PROFILE_INJECTIONS` template markers are now present in `templates/context/` (Remediated: 2026-05-26).
