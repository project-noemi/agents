...
- **Gatekeeper Implementation Gap**: The reference implementation in `examples/gatekeeper-deployment/` does not yet execute the full mutating action set (merges/closes) described in the persona. Tracked for resolution via Decision [2026-06-12-0005] (dry-run mode).
...
- **Resilience Helper Integration Gap** (Closed: 2026-06-19): Reaffirmed as reference-only by Decision [2026-06-19-0004]. `scripts/resilience_helpers.js` is intentionally not force-integrated into deterministic local-FS tools.
- **Resilience Helper Module System Mismatch** (Remediated: 2026-06-26): ESM consumers can now use `scripts/resilience_helpers.mjs` and `scripts/audit_logger.mjs` for named-import access to the CommonJS implementations (Decision [2026-06-26-0004]).
...
- **Golden Fixture Coverage Gap (Value Lenses & Operating Profiles)** (Remediated: 2026-06-12): All six template injection markers are now covered by `tests/golden-fixtures.test.js` and `scripts/update-golden-fixtures.js` (Decision [2026-06-12-0016]). New fixture files `tests/fixtures/generated/value-lenses.md` and `tests/fixtures/generated/operating-profiles.md` close the regression safety net.
- **Test Harness Skills Coverage Asymmetry** (Remediated: 2026-06-12): `tests/contracts.test.js` now includes a skill contract test symmetrical to the persona test (Decision [2026-06-12-0017]). Skill contract compliance is enforced in both `npm run audit` and `npm test`.
- **Gatekeeper Config Schema Absent** (Remediated: 2026-06-12): A `.gatekeeper/config.yml.example` documenting the full YAML schema (repo allowlist, file-pattern overrides, diff-size thresholds, conflict grace period) is now present, and `agents/engineering/gatekeeper.md` references it (Decision [2026-06-12-0018]).
- **Audit Script Coverage Expansion Pending**: Decision [2026-06-12-0007] batches six planned audit-script enhancements (Node baseline check, config-to-asset mapping, naming convention regex, Refusal Criteria substantive check, AI model baseline check, skill-to-agent reference integrity) into a single follow-up PR. Until shipped, the underlying drifts (Node 24 in Docker, mcp.config.json validity, naming convention enforcement, AI model pinning enforcement, skill-to-agent reference integrity) remain open.
- **Substantive Persona/Skill Drift Backlog**: Per Decision [2026-06-12-0006], substantive remediation of placeholder `Data Inventory`, `Refusal Criteria`, and `Audit Log` sections proceeds incrementally — each persona or skill exits its next PR in substantive compliance. The current count of placeholder files is the rolling backlog metric.
- **Red Team Gauntlet Test Vectors** (Remediated: 2026-06-14): A starter `examples/red-team-gauntlet/test-vectors.yaml` enumerating the three PromptShield and two PIIGuard cases mandated by `agents/operations/client-onboarding.md` is now present (Decision [2026-06-12-0003]). The starter set is illustrative; production deployers extend it with their own organizational vectors.
- **Gatekeeper Mutating Actions** (Remediated: 2026-06-26): Decision [2026-06-12-0005] is implemented. `examples/gatekeeper-deployment/entrypoint.sh` honors `GATEKEEPER_MUTATING_MODE` (default `false`) with `auto-merge` and `close-stale` classifications, gated by the existing `GATEKEEPER_DRY_RUN` (default `true`). See Decision [2026-06-26-0005].
- **Casdoor Reference Skill** (Partially Remediated: 2026-06-26): `skills/security/casdoor-validate.md` is shipped (Decision [2026-06-26-0006]). The optional middleware sample in `dashboard-ingest.js` (gated on `CASDOOR_VALIDATE_TOKENS=true`) remains a small open implementation task tracked here.
- **Agent Index First-Paragraph Extraction** (Remediated: 2026-06-26): `scripts/context_helpers.js` now extracts the full first paragraph of `## Role`, closing the prior first-sentence truncation. See Decision [2026-06-26-0007]. Closes the `Agent Index Accuracy Drift` item.
- **RFP Split Naming Remediation** (Remediated: pre-2026-06-26): Files in `examples/rfp-split/` already use slug-based naming (`section-N-...-...`); the prior entry was overtaken by events. Decision [2026-06-12-0013] is closed.
- **Generator Script Consolidation** (Remediated: pre-2026-06-26): `scripts/generate_gemini.js` and `scripts/generate_claude.js` are already thin shims that delegate to `scripts/generate_all.js`. Decision [2026-06-12-0015] is closed.
- **Internal Tool / Build Utility Audit Log** (Partially Remediated: 2026-06-26): `examples/gatekeeper-deployment/dashboard-ingest.js` now emits structured stderr NDJSON via `scripts/audit_logger.js` on every ingest attempt (Decision [2026-06-26-0008]). `tools/executive-assistant/` adoption remains the next concrete step (uses ESM bridge `scripts/audit_logger.mjs` from Decision [2026-06-26-0004]).
- **Phase 0 Assessment Kit Audit Coverage** (Remediated: 2026-06-26): `scripts/audit-repo.js` now verifies the nine canonical Phase 0 files exist (Decision [2026-06-26-0003]). A rename or deletion fails the audit.
- **Executive Assistant Learning Agent Pattern** (Documented: 2026-06-26): The `/api/resolution` feedback loop in `tools/executive-assistant/server.js` is recognized as a documented pattern of the Executive Assistant persona, not a distinct agent. No new persona file is required (Decision [2026-06-26-0001]).
- **Mandatory Heading Inconsistency** (Closed: 2026-06-26): The differing `Rules & Constraints` vs. `Rules & Constraints (4D Diligence)` heading text between persona and skill contracts is intentional. The audit script uses `scripts/context_helpers.js` constants as the single source of truth (Decision [2026-06-26-0002]).
