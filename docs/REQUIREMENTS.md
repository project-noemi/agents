...
### 10. Distribution Model: Fair Source with the Transparent Source Guarantee

- The repository is licensed under the **Functional Source License, FSL-1.1-Apache-2.0** (Decision [2026-07-05-0004]): each published version is freely readable, usable, modifiable, and redistributable, with a two-year restriction on offering a competing commercial NoéMI-as-a-service, after which that version automatically converts to Apache-2.0.
- The public distribution-model label is **Fair Source** (fair.io), declared via the SPDX `license` field in `package.json`. Self-referential claims use "Fair Source", not "open source"; references to third-party open-source components (Infisical, Casdoor, self-hosted models) are unchanged.
- [`TRANSPARENT_SOURCE.md`](TRANSPARENT_SOURCE.md) is the canonical definition of the **NoéMI Transparent™** mark and its five-clause guarantee, scoped to spec-and-governance transparency (deterministic context generation, golden fixtures, mandatory persona Audit Log contracts, public decision log) — not model-weight or training-data disclosure.
- **Tagged GitHub releases are the canonical published versions** that anchor the rolling two-year FSL-to-Apache-2.0 conversion timeline; a tagged release must exist at (or after) the relicense merge commit.

### 11. Docker Guidance Must Describe the Home, Not a Fake Runtime
...
- **Automation Branch Flow Undocumented Under the develop-only Merge Gate**: Decision [2026-07-03-0001] makes `develop` the only valid PR source into `main` (the `doc-*` wildcard exceptions were removed from `.github/workflows/require-develop-source.yml`). Governed automation runs on `claude/*` session branches and `sync/upstream-*` branches whose PRs must therefore target `develop`, but Requirement §7 and the automation prompt docs (`docs/DEV_AGENT_PROMPT.md`, `docs/SYNC_AGENT_PROMPT.md`) do not yet document this integration path (see Clarification [2026-07-05] Automation Branch Flow).
...
- **FSL Conversion Clock Not Anchored**: Requirement §10 and Decision [2026-07-05-0004] make tagged releases the canonical published versions that start the two-year FSL-to-Apache-2.0 conversion timer, but the repository's only tag (`v0.1.0`, at the PR #162 merge) predates the relicense commit (`c3c0e82`). No post-relicense tagged release exists, so the first conversion clock has no legally unambiguous start date (see Clarification [2026-07-07] FSL Conversion Clock).
...
- **Operating Profile Starter Set Partially Implemented**: `operating-profiles/standard-operating-profile.md` now exists and is injected into generated context, but the other two profiles approved by Decision [2026-06-19-0011] (`high-trust-regulated.md`, `fast-iteration-startup.md`) remain unwritten, and the JSON sovereign profile stays invisible to the Markdown-only generator. (The `templates/tiers/` half of the former "Infrastructure Asset Implementation Gap" was remediated — see the archive below.)
- **Ungoverned Fleet-Wide Value Injection (`compassion-lens.json`)**: commit `bb381d2` added `value-lenses/compassion-lens.json` with the stated intent to "inject these values fleet-wide" (Anti-Replacement rule, precarity consciousness), and Decision [2026-07-05-0003] cites its shape as the pattern for JSON lens companions — yet the generator injects only `.md` lenses (`scripts/context_helpers.js` filters `endsWith('.md')`), the lens is absent from the `value-lenses/README.md` directory pattern and rollout list, it has no `LENS_TEMPLATE.md`-shaped Markdown contract, and the Anti-Replacement rule it enforces is documented nowhere in this file or `AGENTS.md`. The fleet-wide injection therefore silently does not happen (see Clarification [2026-07-07] Compassion Lens Governance).
- **Guardian Layer Executable Path Is Python-Only**: Requirement §7 declares Guardian-layer patterns a core architectural requirement, and `guardian-layer/guardian_evaluator.py` now provides the only executable implementation ("AI evaluates AI" middleware interception) — but it is labeled LEGACY/ILLUSTRATIVE per §8, and no canonical Node.js implementation exists on the repository's primary runtime path (see Clarification [2026-07-07] Guardian Layer Canonical Path).
...
- **Service Tier Template Gap** (Remediated: 2026-07-05, verified 2026-07-07): `templates/tiers/` now contains substantive `basic.md`, `standard.md`, and `premium.md` service tier definitions (commit `9c19548`, salvaged from stale PR #254), unblocking the Client Onboarding workflow. Clarifications [2026-06-10] and [2026-06-15] were closed as overtaken by events (Decision [2026-07-07-0001]).
