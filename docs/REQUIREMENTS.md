...
- **Internal Tool Observability Gap**: While the shared `scripts/audit_logger.js` utility is available, Node.js tools like `executive-assistant` (`tools/executive-assistant/server.js`) and reference services like `dashboard-ingest.js` still rely on unstructured `console.log` (lines 44, 49, 121, 161) and `console.error` (line 54) and have not yet adopted the mandated JSON Audit Log emission to `stderr`. This includes events like `[Pub/Sub] Notification received` and `[Triage Router] Execution Complete`.
