...
- **Docker Smoke Default Divergence**: `.env.template` documents `NOEMI_DOCKER_SMOKE_TIMEOUT_MS=60000`, `NOEMI_DOCKER_SMOKE_POLL_INTERVAL_MS=5000`, and `NOEMI_DOCKER_SMOKE_ARTIFACT_DIR=./tests/e2e/artifacts`, while `tests/e2e/docker-smoke.test.js` falls back to `600000`, `2000`, and `test-artifacts/docker-smoke` respectively. Users following the template inherit a 10× shorter timeout (risking false failures on compose pulls) and an artifact path that is not covered by `.gitignore` (which only ignores `test-artifacts/`), polluting the working tree.
...
- **Branch Governance Enforcement Drift**: Requirement §7 mandates a canonical `develop → main` flow, but no `develop` branch exists at origin. The CI gate `.github/workflows/require-develop-source.yml` allowlists `develop` plus a growing set of hardcoded automation branch prefixes (`doc-workflow-audit-drift*`, `doc-requirements-uplift*`, etc., now also `claude/*`), each added by editing the security gate itself. The effective merge flow is automation branches → `main`, contradicting the documented governance model.
