...
- **Branching Invariant**: The canonical flow is `feature/* → develop → main`. `develop` must contain everything on `main` at all times. All pull requests to `main` MUST originate from the `develop` branch (Decision [2026-07-03-0001]). Automation and feature branches must target `develop` as their PR destination.
