...
- **Merge-Gate Wildcard Re-Accretion and develop/main Divergence** (Answered 2026-07-10-0002; implementation pending on the reconciling release PR): Requirement §7 mandates that `develop` is the only valid PR source into `main` and that any hotfix exception must be "a single explicit entry... never a broad wildcard," but `main`'s copy of `.github/workflows/require-develop-source.yml` has re-accreted two wildcard exceptions (`doc-requirements-uplift-*` and `doc-requirements-refinement-*`) via direct-to-main automation commits `a0a7fad` (2026-07-07) and `e54275a` (2026-07-09) — after Decision [2026-07-03-0001] removed the previous six. Decision [2026-07-10-0002] answers this: (a) the wildcards are drift and must be removed on the reconciling release PR; (b) doc-automation must base and target `develop` per Decision [2026-07-07-0002]; (c) the initial `develop → main` reconciliation is human-authored this cycle, then a weekly scheduled routine takes over per §7. Implementation sub-tasks still open: **(i)** author the human-driven `develop → main` release PR that resolves the diverged governance-doc state (later timeline from `develop` wins) and strips the two wildcards from `.github/workflows/require-develop-source.yml`; **(ii)** identify and retarget the automation trigger that produced the direct-to-main branches on 2026-07-07 and 2026-07-09 (GitHub Action config, scheduled agent, or external bot) so future runs base off `develop`; **(iii)** enable the weekly scheduled `develop → main` release routine.
...
- **Licensing Posture Not Codified**: The repository was relicensed from SSPL v1 to **FSL-1.1-Apache-2.0** (commit `c3c0e82`) with the Transparent Source Guarantee added in `docs/TRANSPARENT_SOURCE.md` (commit `7f756b6`, Decision [2026-07-05-0004]), but this document contains no licensing requirement, and `scripts/audit-repo.js` does not verify that `LICENSE`, the `package.json` `license` field, and `docs/TRANSPARENT_SOURCE.md` stay present and consistent. The licensing posture — a first-class governance control for a public reference architecture — currently exists only in the decision log and ancillary docs (see Clarification [2026-07-11] Licensing Posture Missing from the Requirements Contract).
- **Sync-Upstream Regression Harness Outside the Validation Gate**: `scripts/test-sync-upstream.sh` (shipped by Decision [2026-06-30-0001]) verifies override surfacing, `--continue` resume, dry-run, and the duplicate guard for `scripts/sync-upstream.sh` offline with a stubbed `gh`, and is documented in `README.md` and `docs/UPSTREAM_SYNC.md` — but neither `package.json` scripts nor `.github/workflows/validate.yml` execute it, so a regression in the upstream-sync script would ship without any CI signal, contrary to the "false green" prevention intent of Requirement §9 (see Clarification [2026-07-11] Sync-Upstream Test Harness Outside the Validation Gate).
...
- **Audit Log Schema Validation Gap**: `scripts/audit-repo.js` (line 92) verifies that Audit Logs are valid JSON but does not yet implement the mandated JSON schema validation (checking for `task`, `inputs`, `actions`, `risks`, and `result`), despite the requirement in §3.
...
- **Red Team Vector JSON Companion Divergence**: `examples/red-team-gauntlet/test-vectors.json` (salvaged in commit `9c19548`) has drifted from the canonical `test-vectors.yaml` (Decision [2026-07-07-0003]): it uses a divergent vector ID scheme (`PI-001` vs. the YAML's `pi-direct-override`), a different per-vector field shape, self-describes as "Consumed by the Client Onboarding validation suite" (the persona and README both name the YAML), and declares `"$schema": "./test-vectors.schema.json"` — a file that does not exist in the repository. The dangling `$schema` reference is a Referential Integrity (D4) violation (see Clarification [2026-07-13] Red Team Vector JSON Companion Divergence and Missing Schema).
