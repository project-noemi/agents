...
- **Memory-Code Synchronization Drift**: Multiple architectural remediations recorded as "completed" in `DECISION_LOG.md` (e.g., sync script parameterization, agent index paragraph extraction, and the shared `audit_logger.js` utility) are physically absent or reverted in the current codebase state.
- **Substantive Skill Library Drift**: 100% of active reusable skills in `skills/` (8/8) currently contain "TBD" placeholders for mandatory `Data Inventory` and `Refusal Criteria` sections, failing substantive compliance with the 4D framework despite passing structural audits.
- **Audit Script Schema Blindness**: `scripts/audit-repo.js` performs basic structural checks but lacks mandatory JSON schema validation for `Audit Log` sections (missing checks for `task`, `inputs`, `actions`, `risks`, and `result` keys).
- **Audit Script Referential Integrity Gap**: The audit utility does not verify that skills or MCPs referenced in `mcp.config.json` or agent workflows exist as physical files in the repository.
- **Internal Tool Observability Gap**: Node.js tools (`tools/executive-assistant`) and reference services (`examples/gatekeeper-deployment/dashboard-ingest.js`) lack structured JSON Audit Log emission to `stderr`, violating the project's observability standard.
- **Agent Index Richness Drift**: `scripts/context_helpers.js` truncates agent roles to the first sentence, leading to potentially misleading summaries in the generated `GEMINI.md` and `CLAUDE.md` context files.
- **Docker Smoke Test Variable Gap**: `tests/examples-smoke.test.js` lacks the mandated validation for `NOEMI_DOCKER_SMOKE_*` environment variables required by Section 9.
...
- **Logging Protocol Implementation Gap**: `logging-mcp` is a dual-backend draft (Loki/n8n) but is not yet active in `mcp.config.json`, and reference services lack alignment with its schema.
- **Branch Protection Audit Gap**: `scripts/audit-repo.js` lacks the mandated logic to verify branch protection status, as required by Section 7.
- **Resilience Helper Integration Gap**: `scripts/resilience_helpers.js` exists as a canonical reference but is not utilized by repository tools or agent personas.
