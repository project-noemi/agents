...
- **CI/CD "False Green" Risk in E2E**: `tests/e2e/docker-smoke.test.js` (lines 12-18) silently skips runtime checks if Docker is absent. This masks environmental gaps in CI/CD when `FORCE_DOCKER_SMOKE=true` is not set.
...
- **Internal Tool Observability Gap**: While the shared `scripts/audit_logger.js` utility is available, Node.js tools like `executive-assistant` (`tools/executive-assistant/server.js`) and reference services like `dashboard-ingest.js` still rely on unstructured `console.log` (lines 44, 49, 121, 161 in `tools/executive-assistant/server.js`) and have not yet adopted the mandated JSON Audit Log emission to `stderr`. This includes events like `[Pub/Sub] Notification received` and `[Triage Router] Execution Complete`.
...
- **Heading Inconsistency**: The mandatory Diligence heading is documented as "Rules & Constraints" in agents but "Rules & Constraints (4D Diligence)" in skills. Codebase scan confirms 90% of agents have already adopted the `(4D Diligence)` suffix, creating a drift from the primary requirement documentation.
- **Undocumented "Learning Agent" Pattern**: `tools/executive-assistant/server.js` implements a `/api/resolution` feedback loop (lines 101-118) and "Learning Agent" logic (`event: 'LEARNING_AGENT_UPDATE'`) that maps human feedback back into the system's execution logs. This pattern lacks a persona specification and governance requirements in `REQUIREMENTS.md`.
- **Undocumented Admin Control Surface**: `tools/executive-assistant/server.js` implements several admin-facing API endpoints (`/api/queue`, `/api/resolution`, `/api/rules`, `/api/stats`, `/api/logs`) that provide unauthenticated access to internal agent state, queued tasks, and rule configurations, creating a governance drift from the security baseline. The `/admin` path (line 19) serves a Vite-based UI from `ui/dist`.

### 3. Substantive & Safety Drift
- **Substantive Content Baseline (The "TBD" Problem)**: 100% of reusable skills (e.g., `skills/reporting/alert-notify.md`) and several framework assets (e.g., `value-lenses/care-continuity.md`) use "TBD" placeholders for mandatory sections like `Data Inventory`, `Refusal Criteria`, or `Owner`, bypassing the D2 (Description) and Refusal Principle mandates.
