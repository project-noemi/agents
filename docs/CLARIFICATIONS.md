...
### ❓ Question [2026-06-11] - Dashboard Ingest Persistence Backend & Schema
**Context:** Requirement §8 ("Reference Examples Must Tell the Truth") states the Gatekeeper example "must use HMAC-signed dashboard submissions and a verifiable ingest path (`/api/v1/reports`)", but the reference service `examples/gatekeeper-deployment/dashboard-ingest.js` implements an entire persistence backend on top of that front door.
**Ambiguity / Drift:** The service reads two undocumented environment variables (`INFLUXDB_WRITE_URL`, `INFLUXDB_AUTH_TOKEN`) and writes each accepted report to InfluxDB as a `cycle_report` line-protocol measurement — tagged by `agent_id`/`org`, with integer fields (`repos_scanned`, `pr_count`, `auto_merged`, `flagged_for_review`, `closed_stale_conflict`, `skipped`, `errors`) and a `mode` field — using nanosecond timestamps. None of this persistence layer or its schema appears in `REQUIREMENTS.md`. A fork operator reading only the requirements cannot provision the backing store or understand the data model the dashboard depends on.
**Question for Product Owner:** Should `REQUIREMENTS.md` (and the example README) formally document the InfluxDB persistence backend, its required environment variables, and the `cycle_report` measurement schema as part of the Fleet Dashboard reference contract — or should the persistence backend be treated as a swappable implementation detail with only the front-door contract guaranteed?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *(For a GitHub Issue labeled `dev work`)* Document the Fleet Dashboard ingest persistence backend in `REQUIREMENTS.md` §8 and `examples/gatekeeper-deployment/README.md`: enumerate the `INFLUXDB_WRITE_URL`/`INFLUXDB_AUTH_TOKEN` variables and the `cycle_report` measurement schema (tags, fields, timestamp precision) emitted by `dashboard-ingest.js`.

### ❓ Question [2026-06-11] - Canonical Exponential-Backoff Parameters
**Context:** Requirement §8 mandates "at least one reusable reference pattern for exponential backoff and retry (Node.js implementation: `scripts/resilience_helpers.js`)" but pins no concrete parameter values.
**Ambiguity / Drift:** The two reference backoff implementations disagree. `scripts/resilience_helpers.js` uses `maxRetries=5`, `baseDelayMs=1000`, `maxDelayMs=30000` (30 s cap), and `jitterFactor=0.2` (jitter applied). The shell `retry_with_backoff` in `examples/gatekeeper-deployment/entrypoint.sh` uses 5 attempts, a 1 s base with ×2 growth, but caps at **16 s** and applies **no jitter**. Because neither the delay ceiling nor the jitter policy is specified in the requirements, the "reference pattern" is ambiguous and forks may copy whichever values they encounter first, producing inconsistent retry behavior across the fleet.
**Question for Product Owner:** What are the canonical backoff parameters the reference pattern must enforce (max attempts, base delay, maximum delay cap, and whether jitter is required)? Should the shell implementation be reconciled to the Node helper's 30 s cap and jitter, or vice versa?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *(For a GitHub Issue labeled `dev work`)* Define the canonical backoff parameters in `REQUIREMENTS.md` §8 and reconcile `examples/gatekeeper-deployment/entrypoint.sh` (currently 16 s cap, no jitter) with `scripts/resilience_helpers.js` (30 s cap, jitter 0.2) so both reference implementations match the agreed values.

### ❓ Question [2026-06-11] - Dashboard Ingest HTTP & Retry-Classification Contract
**Context:** Requirement §8 names the `/api/v1/reports` ingest path as "verifiable" but does not specify its HTTP response semantics or how clients should react to each status.
**Ambiguity / Drift:** `dashboard-ingest.js` implements a specific, undocumented contract: `202 Accepted` on success, `400` on malformed JSON, `401` on a missing Bearer token or invalid HMAC signature, `404` for unknown routes, `502` when the InfluxDB write fails, plus a `GET /health` → `200` liveness endpoint. The client `entrypoint.sh` mirrors this with a retry classification that treats `401` as **fatal (no retry)** while retrying `429`, any `5xx`, and network failures (`000`). This split — which errors are permanent vs. transient — is critical for correct fleet behavior but exists only in code.
**Question for Product Owner:** Should the ingest endpoint's HTTP status contract (including `/health`) and the client-side retry classification (`401` fatal vs. `429`/`5xx`/network retryable) be promoted to a documented requirement, so alternative dashboard implementations and clients remain interoperable?
**Answer:** [LEAVE BLANK FOR HUMAN TO FILL]
**🤖 Jules Action Prompt:** *(For a GitHub Issue labeled `dev work`)* Document the Fleet Dashboard ingest HTTP contract (status codes `202`/`400`/`401`/`404`/`502` and `GET /health`) and the client retry classification (fatal vs. transient) in `REQUIREMENTS.md` §8, and add a smoke assertion in `tests/examples-smoke.test.js` that pins these response codes.
