...
## [2026-07-08-0001] Service Tier Templates Shipped — Q [2026-06-10] and Q [2026-06-15] Resolved by Implementation

- **Decision:** CLARIFICATIONS.md Q [2026-06-10] (Empty Tier Templates Implementation Gap) and Q [2026-06-15] (Service Tier Template Specifications) are closed as **resolved by implementation**. Commit `9c19548` ("feat(assets): salvage gap-filling assets from stale PR #254") landed `templates/tiers/basic.md`, `templates/tiers/standard.md`, and `templates/tiers/premium.md` on `develop`; the three files together define the Basic/Standard/Premium tier surfaces (provisioned MCPs, provisioned skills, onboarding validation suite, guardrails, retention/observability, and upgrade path) that the `Client Onboarding` agent references at `agents/operations/client-onboarding.md:63`. The commercial-definition question that Decision [2026-06-19-0009] originally deferred to the Product Owner is now moot for the reference architecture: the three shipped files ARE the reference definitions, released under the same FSL-1.1-Apache-2.0 terms as the rest of the repository (Decision [2026-07-05-0004]). Downstream deployers who need different commercial tier definitions extend these files rather than the empty-directory posture the clarifications were filed against.
- **Context:** Both clarifications carried a PO-deferred flag from Decision [2026-06-19-0009] because the *commercial* tier definitions were considered PO territory. The shipped files chose the pragmatic path: they define tier surfaces in terms the reference architecture already owns (MCP allowlists, skill allowlists, onboarding gates, refusal defaults) rather than in the commercial terms (pricing, SLAs, contracted limits) that would genuinely require PO input. That framing keeps the tier templates truthful reference documentation without over-committing on commercial specifics. Verified during resolution: all three files exist (`templates/tiers/basic.md`, `standard.md`, `premium.md`); the `Client Onboarding` persona resolves its `templates/tiers/*` reference to real files; and each file follows a consistent shape (Positioning, Provisioned MCPs, Provisioned Skills, Onboarding Validation Suite, Guardrails, Retention/Observability, Upgrade Path).
- **Impact:** CLARIFICATIONS.md shrinks from 15 to 13 open questions (two removed). `REQUIREMENTS.md` Current Known Limitation entry "Infrastructure Asset Implementation Gap" is updated to reflect that `templates/tiers/` is now populated with `basic.md`, `standard.md`, and `premium.md`; the remaining gap in that entry is the two unimplemented Operating Profiles from Decision [2026-06-19-0011] (`high-trust-regulated.md`, `fast-iteration-startup.md`) plus the invisible-to-generator JSON sovereign profile tracked separately under the Sovereign JSON Asset Layer Governance Gap. Decision [2026-06-19-0009] is annotated as superseded-by-implementation for these two clarifications; commercial-specifics questions (pricing, contractual SLAs) are not raised by this closure and remain a future PO topic if ever needed.
