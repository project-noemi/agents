...
- **Audit Script Referential Integrity Gap** (Decision [2026-06-18-0001], tracked as `REQ-AUDIT-REFINT`): `scripts/audit-repo.js` must add a referential-integrity pass that validates (1) `mcp.config.json` `active_mcps` and `active_skills` entries map to real files under `mcp-protocols/` and `skills/`, (2) every `**Skill:** [path]` reference inside agent personas resolves to a tracked `skills/*.md` file, and (3) every internal markdown link in `docs/`, `agents/`, `skills/`, and root mandate files resolves to a tracked file. CI runs treat failures as fatal; local runs surface them as warnings. This consolidates Q [2026-05-02] (Config-to-Asset Mapping Validation), Q [2026-05-15] (Skill-to-Agent Referential Integrity), and Q 2026-06-17 (Automated Internal Documentation Link Integrity).
- **Internal Tool Audit Log Mapping** (Decision [2026-06-18-0002], tracked as `REQ-INTERNAL-AUDIT-LOG-IMPL`): Internal Node.js tools (`tools/executive-assistant/`) and reference services (`examples/gatekeeper-deployment/dashboard-ingest.js`) must emit JSON Audit Logs to `stderr` using the canonical 5-key schema (`task`, `inputs`, `actions`, `risks`, `result`) with the event-to-schema mapping fixed by Decision [2026-06-18-0002] (slug-style `<tool>.<event>` `task` identifier, sanitized `inputs`, discrete `actions`, identified `risks`, terminal `success`/`rejected:<reason>`/`error:<reason>` `result`). Tools MUST adopt the shared `scripts/audit_logger.js` utility once it lands; until then direct `process.stderr.write` of the JSON is acceptable.
