...
- The public documentation must include a reusable **Phase 0 Assessment Kit** containing exactly 9 mandated files:
...
- **Internal Tool Observability Gap**: While the shared `scripts/audit_logger.js` utility is available, Node.js tools like `executive-assistant` (`tools/executive-assistant/server.js`) and reference services like `dashboard-ingest.js` still rely on unstructured `console.log` (lines 44, 49, 121, 161) and have not yet adopted the mandated JSON Audit Log emission to `stderr`. This includes events like `[Pub/Sub] Notification received` and `[Triage Router] Execution Complete` (lines 44, 49, 121, 161).
...
- **Undocumented "Learning Agent" Pattern**: `tools/executive-assistant/server.js` (lines 107-124) implements a `/api/resolution` feedback loop and "Learning Agent" logic (`event: 'LEARNING_AGENT_UPDATE'`) that maps human feedback back into the system's execution logs. This pattern lacks a persona specification and governance requirements in `REQUIREMENTS.md`.
- **Undocumented Admin Control Surface**: `tools/executive-assistant/server.js` implements several admin-facing API endpoints that provide unauthenticated access to internal agent state, queued tasks, and rule configurations, creating a governance drift from the security baseline. These include:
  - `/admin` serving a Vite-based UI from `ui/dist` (line 19)
  - `/api/queue` (line 103)
  - `/api/resolution` (line 107)
  - `/api/rules` (lines 125, 129)
  - `/api/stats` (line 150)
  - `/api/logs` (line 156)
