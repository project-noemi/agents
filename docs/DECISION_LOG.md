...
## [2026-07-06-0001] Service Tier Templates — Shipped; Clarifications Closed as Overtaken by Events

- **Decision:** CLARIFICATIONS.md Q [2026-06-10] "Empty Tier Templates Implementation Gap" and Q [2026-06-15] "Service Tier Template Specifications" are closed as **overtaken by events**: `templates/tiers/basic.md`, `standard.md`, and `premium.md` now exist with substantive content (commit `9c19548`, "salvage gap-filling assets from stale PR #254", merged via PR #264 on 2026-07-05). Each template defines tier positioning, provisioned MCPs, provisioned skills, onboarding checks, and graduation criteria — the technical service definitions the questions asked for. The templates deliberately contain **no pricing**: consistent with Decision [2026-06-19-0009], commercial pricing remains a Product Owner decision and stays outside the public reference architecture unless the PO explicitly requests its inclusion.
- **Context:** Decision [2026-06-19-0009] had deferred these templates pending PO-supplied service definitions. The salvaged PR #254 assets supplied technically grounded definitions (feature/MCP/skill/limit scope, no pricing), unblocking the `Client Onboarding` agent's workflow references without preempting the commercial decision. Verified 2026-07-06: all three files present with tier-specific substantive content; `templates/tiers/README.md` documents the directory's tracked status.
- **Impact:** Both questions are removed from the active backlog. The "Infrastructure Asset Implementation Gap" limitation in `REQUIREMENTS.md` is split: the tier-template portion moves to the Remediated archive, and the remaining operating-profile portion is narrowed to the two unshipped starter profiles from Decision [2026-06-19-0011] (`high-trust-regulated.md`, `fast-iteration-startup.md`).

### Archived Q&A — Q [2026-06-10] Empty Tier Templates Implementation Gap

- **Question:** Should Jules implement a starter set of service tier templates in `templates/tiers/` based on the documented `basic`, `standard`, and `premium` tiers?
- **Resolution (2026-07-06):** Overtaken by events — templates shipped in PR #264 (commit `9c19548`) before a PO answer was recorded.

### Archived Q&A — Q [2026-06-15] Service Tier Template Specifications

- **Question:** Can you provide the specific service definitions (features, limits, pricing) for the Basic, Standard, and Premium tiers so I can implement the missing templates?
- **Resolution (2026-07-06):** Overtaken by events — technical service definitions (features, MCP/skill provisioning, limits, graduation criteria) shipped in PR #264; pricing intentionally omitted per Decision [2026-06-19-0009].
