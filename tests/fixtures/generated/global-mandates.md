...
- **AI Model Baseline**: Reference workflows, lab examples, and smoke tests are pinned to **Gemini 3.6 Flash** (`models/gemini-3.6-flash`) as the canonical baseline for predictable performance and cost. This baseline governs labs and smoke tests only — the cross-model review floor is **`pro`** (`GEMINI_REVIEW_FLOOR`, Decision [2026-08-15-0003]); do not lower the review floor to "align" it with the lab baseline.
