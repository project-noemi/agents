...
- **Undocumented Admin Control Surface**: `tools/executive-assistant/server.js` (lines 19, 103, 107, 121, 125, 131, 150, 156) implements several admin-facing API endpoints (`/api/queue`, `/api/stats`, `/api/logs`, `/api/rules`, `/api/resolution`) that provide unauthenticated access to internal agent state, queued tasks, and rule configurations, creating a governance drift from the security baseline. The unauthenticated `/admin` path (line 19) serves a Vite-based UI from `ui/dist`.

### 3. Substantive & Safety Drift
- **Substantive Content Baseline (The "TBD" Problem)**: 100% of reusable skills (e.g., `skills/reporting/alert-notify.md`) and most agent personas use "TBD" placeholders or boilerplate for `Data Inventory` and `Refusal Criteria`, bypassing the D2 (Description) and Refusal Principle mandates. Several core framework assets (e.g., `value-lenses/care-continuity.md`, `value-lenses/balanced-enterprise.md`, `value-lenses/performance-efficiency.md`) similarly rely on `TBD` placeholder values (e.g., for owner details).
