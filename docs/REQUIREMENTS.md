...
- **Heading Inconsistency**: The mandatory Diligence heading is "Rules & Constraints" in agents but "Rules & Constraints (4D Diligence)" in skills, complicating automated audit logic. Additionally, there is a documentation-code drift where approximately 90% of active agent personas (e.g., `linux.md`, `student-success-coach.md`) actually use `## Rules & Constraints (4D Diligence)` as their heading, contradicting the `AGENTS.md` mandate for `## Rules & Constraints`.
